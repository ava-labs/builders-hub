...
async function handleGetRelayers(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (!response.ok) {
      const message =
        (await extractServiceErrorMessage(response)) || "Failed to fetch relayers from Builder Hub";
    ...
    console.log(
      `[Relayers] Total from API: ${allRelayers.length}, Filtered for user ${userId}: ${relayers.length}`
    );
    ...
    return jsonError(
      500,
      error instanceof Error ? error.message : "Failed to fetch relayers",
      error
    );
  }
}
...
async function handleCreateRelayer(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (!response.ok) {
      const message =
        (await extractServiceErrorMessage(response)) || "Failed to create relayer in Builder Hub";
    ...
    return jsonError(
      500,
      error instanceof Error ? error.message : "Failed to create relayer",
      error
    );
  }
}
