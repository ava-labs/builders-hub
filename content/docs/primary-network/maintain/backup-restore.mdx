...
NodeID is a unique identifier that differentiates your node from all the other peers on the network. It's a string formatted like `NodeID-5mb46qkSBj81k9g9e4VFjGGSbaaSLFRzD`. You can look up the technical background of how the NodeID is constructed [here](/docs/apis/standards/cryptographic-primitives#tls-addresses). In essence, NodeID is defined by two files:
...
<Callout type="warn">
If you have users defined in the keystore of your node, then you need to back up and restore those as well. The keystore provides methods to export and import user keys. Note that the keystore is used by developers only and not intended for use in production nodes. If you don't know what a keystore API is and have not used it, you don't need to worry about it.
</Callout>
...
You can now check that the node is restored with the correct NodeID and NodePOP by issuing the [getNodeID](/docs/apis/info-api#infogetnodeid) API call in the same console you ran the previous command:
