...
- Built `avalanchego` and `tmpnetctl` binaries
- Current directory is the avalanchego repository root

## Start Your First Network

### Basic Start Command

Start a 2-node network (the minimum for consensus):

```bash
cd /path/to/avalanchego

# Start network with absolute path
...
```bash
./build/tmpnetctl stop-network    # Uses TMPNET_NETWORK_DIR
./build/tmpnetctl restart-network
```
