...
export async function triggerCertificateWebhook(
  ...
  try {
    ...
    if (!codebaseCourseIds.includes(courseId)) {
      // Skipping HubSpot webhook for non-Codebase course
      return;
    }
    ...
    // Don't throw on webhook failure - certificate generation should succeed regardless
  } catch (error) {
