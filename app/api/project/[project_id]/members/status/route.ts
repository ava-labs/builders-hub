...
export const PATCH = withAuth(async (request: Request, context: any, session: any) => {
  try {
    ...
    const { user_id, status, email: _email, wasInOtherProject } = body;
    ...
    if (user_id !== null && user_id !== undefined && user_id !== '' && user_id !== session.user.id) {
      return NextResponse.json({ error: 'Forbidden: You can only update your own member status' }, { status: 403 });
    }
