...
function CreateManagedTestnetRelayerBase() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      setSelectionError(
        "Source and destination cannot be the same network when selecting one each"
      );
      return;
    }
  ...
  useEffect(() => {
    if (createdRelayerResponse && relayers.length > 0) {
      ...
      console.log(
        "Available relayers:",
        relayers.map((r) => ({ id: r.relayerId, created: r.createdAt }))
      );
  ...
  const handleToggleSource = (l1Id: string) => {
    setSelectedSources((prev) =>
      prev.includes(l1Id) ? prev.filter((id) => id !== l1Id) : [...prev, l1Id]
    );
  };
      ...
      for (const config of createdRelayer.configs) {
        try {
          ...
          const balance = await client.getBalance({
            address: createdRelayer.relayerId as `0x${string}`,
          });
  ...
  return (
    <Steps>
      ...
      <Step>
        ...
        <p className="text-sm text-gray-500 mb-8">
          Your relayer has been created. Fund the relayer address on all configured chains to cover
          transaction fees.
        </p>
