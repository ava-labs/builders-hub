...
export default function ShowCaseCard({ projects, events, initialFilters, totalProjects }: Props) {
  const [searchValue, setSearchValue] = useState("");
  const [filters, setFilters] = useState<ProjectFilters>(initialFilters);
  const [currentPage, setCurrentPage] = useState(initialFilters.page ?? 1);
  const [recordsByPage, setRecordsByPage] = useState(initialFilters.recordsByPage ?? 12);
  ...
  const handleFilterChange = (type: keyof ProjectFilters, value: string) => {
    ...
    if (newFilters.recordsByPage) {
      ...
      setTotalPages(Math.ceil(totalProjects / Number(newFilters.recordsByPage)));
    }
  ...
  return (
    <Card className="bg-zinc-50 dark:bg-zinc-950 relative border border-zinc-300 dark:border-zinc-800 p-8">
      <CardHeader className="p-0">
        <CardTitle className="text-2xl text-zinc-900 dark:text-zinc-50">Showcase</CardTitle>
        <CardDescription className="text-zinc-600 text-base dark:text-zinc-400">
          Discover innovative projects built during our hackathons. Filter by track, technology, and
          winners
        </CardDescription>
      </CardHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mt-6">
        <div className="w-full">
          <Tabs
            defaultValue={filters.winningProjecs ? "winingProjects" : "allProjects"}
            className="w-full"
          >
      ...
      <div className="mt-12">
        <h1 className="text-2xl text-zinc-900 dark:text-zinc-50">
          {totalProjects}{" "}
          {totalProjects > 1 ? "Projects" : totalProjects === 0 ? "No projects found" : "Project"}{" "}
          found
        </h1>
        <Separator className="my-8 bg-zinc-300 dark:bg-zinc-800 h-[2px]" />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {projects.map((project, index) => (
            <Link key={index} href={`/showcase/${project.id}`} className="w-full h-full">
