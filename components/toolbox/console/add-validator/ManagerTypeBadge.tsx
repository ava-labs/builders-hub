...
function describe(ownerType: OwnerType, stakingType: StakingType): BadgeDescriptor | null {
  if (ownerType === 'StakingManager' && stakingType === 'native') {
    return { label: 'PoS · Native', tone: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' };
  }
  ...
  if (ownerType === 'PoAManager') {
    return { label: 'PoA · Multisig', tone: 'border-violet-500/30 bg-violet-500/10 text-violet-600 dark:text-violet-400' };
  }
  if (ownerType === 'EOA') {
...
export function ManagerTypeBadge({ ownerType, stakingType, isDetecting, className }: ManagerTypeBadgeProps) {
  if (isDetecting) {
    return (
      <Badge variant="outline" className={`text-[10px] font-normal text-zinc-400 dark:text-zinc-500 ${className ?? ''}`}>
        Detecting…
      </Badge>
    );
  }
