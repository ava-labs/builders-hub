...
} from "@/components/toolbox/console/testnet-infra/ManagedTestnetNodes/types";
  ...
  const createNode = useCallback(async (subnetId: string, blockchainId: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          data.message || data.error || `Error ${response.status}: Failed to register subnet`
        );
      }
