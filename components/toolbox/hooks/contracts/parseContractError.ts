...
const KNOWN_ERRORS: Record<string, string> = {
  // ValidatorManager
  '0xdfae8801': 'Exceeds the maximum churn rate. The network limits how much weight can change at once (typically 20% of total weight). Try a smaller amount or wait.',
  'MaxChurnRateExceeded': 'Exceeds the maximum churn rate. The network limits how much weight can change at once (typically 20% of total weight). Try a smaller amount or wait.',
  '0x5765a929': 'Invalid validator status. The validator may already have a pending operation or may not be active.',
  'InvalidValidatorStatus': 'Invalid validator status. The validator may already have a pending operation or may not be active.',
  ...
  'InvalidBLSKeyLength': 'Invalid BLS public key length. Expected 48 bytes (96 hex chars). Check the validator credentials.',
