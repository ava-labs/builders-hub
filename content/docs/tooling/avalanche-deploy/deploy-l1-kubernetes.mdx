...
<Callout type="info">
The Kubernetes path covers deployment, sync, and L1 creation. Advanced operational workflows (staking key backup, database snapshots, validator migration) are only available in the [Terraform + Ansible path](/docs/tooling/avalanche-deploy/deploy-l1).
</Callout>
