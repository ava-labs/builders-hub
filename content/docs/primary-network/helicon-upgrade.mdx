...
<Callout type="warn" title="A short validation cannot be delegated to">
...
**C-Chain developers.** ACP-194 changes when state is available relative to block acceptance, and several C-Chain RPC namespaces are deprecated alongside it. Read [Continuous Execution](/docs/primary-network/continuous-execution) and the [v1.15.0 release notes](https://github.com/ava-labs/avalanchego/releases/tag/v1.15.0) before assuming existing behavior holds.
