...
Avalanche can run multiple blockchains. Each blockchain is an instance of a [Virtual Machine](/docs/avalanche-l1s/virtual-machines), much like an object in an object-oriented language is an instance of a class. That's, the VM defines the behavior of the blockchain.
...
Before proceeding to add a validator to our Avalanche L1, we will need to have the validator's NodeID, BLS public key and proof of possession. These can be obtained by ssh into the node itself and run the `getNodeID` API specified [here](/docs/apis/info-api#infogetnodeid).
