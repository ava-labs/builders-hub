...
/**
 * Generate meta.json for Cross-Chain documentation
 */
async function generateCrossChainMeta(): Promise<void> {
  const metaContent = {
    ...
    pages: [
      ...
      "avalanche-warp-messaging/overview",
      "avalanche-warp-messaging/deep-dive",
      "avalanche-warp-messaging/evm-integration",
      "avalanche-warp-messaging/run-relayer",
      "---ICM Contracts---",
      "teleporter/overview",
      "teleporter/deep-dive",
      "teleporter/cli",
      "teleporter/upgradeability",
      "teleporter/addresses",
      "---Interchain Token Transfer---",
      "interchain-token-transfer/overview",
      "---Examples---",
      "teleporter/getting-started",
      "teleporter/teleporter-on-local-network",
      "teleporter/teleporter-on-devnet"
    ]
  };

  // Ensure directory exists
  const metaDir = path.dirname('content/docs/cross-chain/meta.json');
  if (!fs.existsSync(metaDir)) {
    fs.mkdirSync(metaDir, { recursive: true });
  }

  // Write meta.json file
  const metaPath = path.join('content/docs/cross-chain/meta.json');
  fs.writeFileSync(metaPath, JSON.stringify(metaContent, null, 2));
  
  console.log(`Generated cross-chain meta.json with ${metaContent.pages.length} pages`);
}

export async function getCrossChainConfigs(): Promise<FileConfig[]> {
  // Generate meta.json
  await generateCrossChainMeta();
