...
import { withApi, ValidationError, NotFoundError, successResponse } from '@/lib/api';
...
function createResponse(
  ...
  const resp = successResponse(data, status);
  resp.headers.set('Cache-Control', CACHE_CONTROL_HEADER);
  resp.headers.set('X-Data-Source', meta.source);
  if (meta.cacheAge !== undefined) resp.headers.set('X-Cache-Age', `${Math.round(meta.cacheAge / 1000)}s`);
  if (meta.fetchTime !== undefined) resp.headers.set('X-Fetch-Time', `${meta.fetchTime}ms`);
  return resp;
}
...
export const GET = withApi(async (request, { params }) => {
  ...
  if (isCacheStale && !revalidatingKeys.has(nodeId)) {
    ...
    (async () => {
      try {
        ...
        if (freshData) {
          cachedData.set(nodeId, { data: freshData, timestamp: Date.now() });
        }
      } finally {
        revalidatingKeys.delete(nodeId);
      }
    })();
    return createResponse({ validatorDetails: cached.data }, { source: 'stale-while-revalidate', nodeId, cacheAge });
  }

  // Return valid cache
  if (isCacheValid && cached) {
    return createResponse({ validatorDetails: cached.data }, { source: 'cache', nodeId, cacheAge });
  }
  ...
  if (!validatorDetails) {
    throw new NotFoundError('Validator not found or not currently active');
  }

  // Update cache
  cachedData.set(nodeId, { data: validatorDetails, timestamp: now });

  return createResponse({ validatorDetails }, { source: 'fresh', nodeId, fetchTime });
});
