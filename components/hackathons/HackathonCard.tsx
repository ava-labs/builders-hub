...
export default function HackathonCard({ hackathon }: { hackathon: HackathonHeader }) {
  return (
    <div
      key={hackathon.id}
      className="flex rounded-lg shadow-lg h-400px md:h-[340px] border border-zinc-300 dark:border-transparent"
    >
      <Link href={hackathon.custom_link ? hackathon.custom_link : `/hackathons/${hackathon.id}`}>
        <Image
          src={
      ...
      <div className="flex-1 justify-evenly bg-zinc-50 dark:bg-zinc-900 border-zinc-300 dark:border-zinc-800 p-6 flex flex-col gap-1 rounded-md md:rounded-l-none">
        <h2 className="uppercase font-bold text-2xl">
          {hackathon.title.length < 45 ? hackathon.title : hackathon.title.slice(0, 40) + "..."}
        </h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs dark:text-zinc-300 text-zinc-800">{hackathon.location}</p>
        </div>
        ...
        <div>
          <p className="text-xs dark:text-zinc-300 text-zinc-800">{hackathon.organizers}</p>
        </div>
        ...
        <Button asChild variant="red" className="w-full py-2 px-4">
          <Link
            ...
            href={hackathon.custom_link ? hackathon.custom_link : `/hackathons/${hackathon.id}`}
