...
function generateSuggestionsFromContent(
  ...
  const relatedSections = sections
    .filter((section) => {
      ...
      return keywords.some(
        (keyword) =>
          contentLower.includes(keyword.toLowerCase()) || titleLower.includes(keyword.toLowerCase())
      );
    })
  ...
  relatedSections.forEach((section) => {
    ...
    if (title.includes("Deploy") || title.includes("Create")) {
      ...
      suggestions.push(
        `Can you explain ${title.toLowerCase().replace("understand", "").replace("learn", "").trim()}?`
      );
    } else {
