  ...
  return (
    <>
      <div className="space-y-4">
        <div className="mb-4">
          This will deploy the <code>TeleporterRegistry</code> contract to the EVM network #
          <code>{selectedL1?.evmChainId}</code>. The contract will be initialized with the
          Teleporter Messenger address <code>{TeleporterMessengerAddress.content.trim()}</code>.
        </div>
        <p className="text-sm text-gray-500">
          Contract source:{" "}
          <a href={TELEPORTER_REGISTRY_SOURCE_URL} target="_blank" rel="noreferrer">
            TeleporterRegistry.sol
          </a>{" "}
      ...
      <Success label="TeleporterRegistry Address" value={teleporterRegistryAddress} />
    </>
  );
}
