...
function ReadPoAManager({ onSuccess }: BaseConsoleToolProps) {
  ...
  async function readContractData() {
    ...
    try {
      ...
      const viewFunctions = PoAManagerABI.abi.filter(
        (item: any) =>
          item.type === "function" &&
      ...
      // Get all events
      const events = PoAManagerABI.abi.filter((item: any) => item.type === "event");
    ...
    <div>
      <div className="space-y-4">
        ...
        <Button
          ...
          loading={isReading}
          disabled={isReading || !poaManagerAddress}
          className="w-full"
        >
          Refresh
        </Button>
      ...
      {Object.keys(viewData).length > 0 && (
        <div className="mt-6">
          <h3 className="text-base font-semibold mb-3 text-zinc-800 dark:text-zinc-200">
            Contract Data
          </h3>
                    ...
                    {logs.length > 0 ? (
                      <div className="space-y-3">
                        {logs.map((log, index) => (
                          <div
                            key={index}
                            className="bg-zinc-50 dark:bg-zinc-800 rounded-md p-3 overflow-x-auto"
                          >
...
export default withConsoleToolMetadata(ReadPoAManager, metadata);
