...
function PlaygroundContent() {
  ...
  const handleColSpanChange = useCallback((chartId: string, newColSpan: 6 | 12) => {
    setCharts((prev) =>
      prev.map((chart) => (chart.id === chartId ? { ...chart, colSpan: newColSpan } : chart))
    );
  }, []);
  ...
  const handleChartTimeFilterChange = useCallback(
    (chartId: string, startTime: string | null, endTime: string | null) => {
      setCharts((prev) => prev.map((c) => (c.id === chartId ? { ...c, startTime, endTime } : c)));
    },
    []
  );
    ...
    const handleDrag = (e: DragEvent) => {
      ...
      if (shouldScroll) {
        ...
        scrollSpeedRef.current =
          START_SCROLL_SPEED + (MAX_SCROLL_SPEED - START_SCROLL_SPEED) * easedFactor;
        scrollAnimationFrameRef.current = requestAnimationFrame(scroll);
      }
    };
    ...
    const loadPlayground = async () => {
      ...
      try {
        ...
        if (playground.charts && Array.isArray(playground.charts)) {
          ...
          setSavedCharts(
            loadedCharts.map((chart: ChartConfig) => ({
              ...chart,
  ...
  const hasChanges = useMemo(() => {
    ...
    if (globalStartTime !== savedGlobalStartTime || globalEndTime !== savedGlobalEndTime) {
      return true;
    }
    ...
    try {
      if (isFavorited) {
        ...
        const response = await fetch(
          `/api/playground/favorite?playgroundId=${currentPlaygroundId}`,
          {
            method: "DELETE",
          }
        );
      ...
      <div className="container mx-auto px-6 py-10 pb-24 space-y-8 relative z-10">
        ...
        <div className="mb-10">
          ...
          {!playgroundId && (
            <p className="text-base text-neutral-600 dark:text-neutral-400 max-w-2xl leading-relaxed">
              Create and customize multiple charts with real-time chain metrics. Add metrics,
              configure visualizations, and share your insights.
            </p>
          )}
          {error && (
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
            ...
            {isOwner && (
              <Popover
                open={showGlobalTimeFilterPopover}
                onOpenChange={setShowGlobalTimeFilterPopover}
              >
                  ...
                  <div className="p-3 border-t space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Input
                        ...
                        value={
                          tempGlobalStartTime
                            ? tempGlobalStartTime.toTimeString().slice(0, 5)
                            : "00:00"
                        }
                        onChange={(e) => {
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Input
                        type="time"
                        value={
                          tempGlobalEndTime ? tempGlobalEndTime.toTimeString().slice(0, 5) : "23:59"
                        }
                        onChange={(e) => {
                          const [hours, minutes] = e.target.value.split(":").map(Number);
                          if (tempGlobalEndTime) {
                    ...
                    <div className="flex gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                      <Button
                        onClick={() => {
                          ...
                          const newStartTime = tempGlobalStartTime
                            ? tempGlobalStartTime.toISOString()
                            : null;
                          const newEndTime = tempGlobalEndTime
                            ? tempGlobalEndTime.toISOString()
                            : null;
            ...
            return (
              ...
              >
                ...
                <ConfigurableChart
                  ...
                  onColSpanChange={
                    isOwner ? (newColSpan) => handleColSpanChange(chart.id, newColSpan) : undefined
                  }
                  onTitleChange={
                    isOwner ? (newTitle) => handleTitleChange(chart.id, newTitle) : undefined
                  }
                  onDataSeriesChange={
                    isOwner
                      ? (dataSeries) => handleDataSeriesChange(chart.id, dataSeries)
                      : undefined
                  }
                  onStackSameMetricsChange={
                    isOwner
                      ? (stackSameMetrics) =>
                          handleStackSameMetricsChange(chart.id, stackSameMetrics)
                      : undefined
                  }
                  onAbbreviateNumbersChange={
                    isOwner
                      ? (abbreviateNumbers) =>
                          handleAbbreviateNumbersChange(chart.id, abbreviateNumbers)
                      : undefined
                  }
                  ...
                  onTimeFilterChange={
                    isOwner
                      ? (startTime, endTime) =>
                          handleChartTimeFilterChange(chart.id, startTime, endTime)
                      : undefined
                  }
