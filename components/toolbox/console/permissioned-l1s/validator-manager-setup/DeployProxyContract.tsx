...
function DeployProxyContract({ onSuccess }: BaseConsoleToolProps) {
  ...
  return (
    <>
      <p className="my-3">
        <a
          href="https://github.com/OpenZeppelin/openzeppelin-contracts/tree/release-v4.9/contracts/proxy/transparent"
          target="_blank"
          rel="noopener noreferrer"
        >
          OpenZeppelin's Transparent Proxy Pattern
        </a>{" "}
        enables upgradeability of smart contracts while preserving state and contract addresses.
      </p>

      <p className="mb-3">
        <strong>How It Works:</strong> The proxy contract stores state and forwards function calls,
        while the implementation contract contains only the logic. The proxy admin manages
        implementation upgrades securely.
      </p>
      ...
      >
        <p>
          If you created your L1 using the <strong>Builder Console</strong>, a proxy contract is{" "}
          <strong>already pre-deployed</strong> at address{" "}
          <code className="bg-amber-100 dark:bg-amber-900/30 px-1 py-0.5 rounded">
            0xfacade0000000000000000000000000000000000
          </code>
        </p>
        <p>You only need this tool if:</p>
        <ul className="list-disc list-inside ml-2 space-y-1">
          ...
          <li>
            You're working with an L1 not created through Builder Console (AvaCloud, Gelato, alt
            BaaS provider)
          </li>
        </ul>
      </AcknowledgementCallout>
        ...
        <Steps>
          <Step>
            ...
            <p className="text-sm text-gray-500">
              This will deploy the <code>ProxyAdmin</code> contract to the EVM network{" "}
              <code>{viemChain?.id}</code>. <code>ProxyAdmin</code> is used to manage upgrades to
              the implementation for the proxy contract. For production L1s this should be a
              multisig wallet, since it can take full control over the L1 validator set by
              arbitrarily changing the implementation of the ValidatorManager contract.
            </p>
            <p className="text-sm text-gray-500">
              Contract source:{" "}
            ...
            <p>
              Deployment Status: <code>{proxyAdminAddress || "Not deployed"}</code>
            </p>
          </Step>
          <Step>
            ...
            <p>
              Deployment Status: <code>{proxyAddress || "Not deployed"}</code>
            </p>
          </Step>
        </Steps>
      </LockedContent>
