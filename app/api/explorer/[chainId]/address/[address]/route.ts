...
async function getNativeBalance(
  ...
  try {
    const balanceHex = (await fetchFromRPC(rpcUrl, "eth_getBalance", [
      address,
      "latest",
    ])) as string;
  ...
  try {
    ...
    for await (const page of result) {
      ...
      for (const txDetails of txDetailsList) {
        ...
        const methodId =
          methodHash && methodHash.startsWith("0x") && methodHash.length === 10
            ? methodHash
            : undefined;
...
export async function GET(
  ...
  try {
    ...
    const [isContractTimed, nativeBalanceTimed, txResultTimed, addressChainsTimed] =
      await Promise.all([
    ...
    if (isContractResult) {
      const metadataTimed = await timed("contractMetadata", () =>
        getContractMetadata(address, chainId)
      );
    ...
    // Log timings
    console.log(
      `[Address API] ${address} on chain ${chainId} - Total: ${totalDuration.toFixed(0)}ms`
    );
    console.log(
      `  Parallel fetch: isContract=${timings.isContract.toFixed(0)}ms, nativeBalance=${timings.nativeBalance.toFixed(0)}ms`
    );
    console.log(
      `  Parallel fetch: transactions=${timings.transactions.toFixed(0)}ms, addressChains=${timings.addressChains.toFixed(0)}ms`
    );
    ...
    const totalDuration = performance.now() - totalStart;
    console.error(
      `[Address API] Error fetching ${address} on chain ${chainId} after ${totalDuration.toFixed(0)}ms:`,
      error
    );
