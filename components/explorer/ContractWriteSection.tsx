...
function getWriteFunctions(abi: ABIFunction[]): ABIFunction[] {
  return abi
    .filter(
      (item) =>
        item.type === "function" &&
...
export default function ContractWriteSection({
  ...
  const writeFunction = async (func: ABIFunction, funcKey: string) => {
    ...
    try {
      ...
      let errorMessage = err instanceof Error ? err.message : error.message || "Transaction failed";
        ...
        <div className="flex items-start gap-3 p-4 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800">
          ...
          <div>
            ...
            <p className="text-xs text-yellow-600 dark:text-yellow-400 mt-1">
              No RPC URL is configured for this chain. Contract write functionality is not
              available.
            </p>
          </div>
        </div>
    ...
    <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
      ...
      <div className="px-4 py-3 bg-green-50/50 dark:bg-green-900/10 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-green-700 dark:text-green-400">
          ...
          <span>
            Connected: {walletEVMAddress.slice(0, 6)}...{walletEVMAddress.slice(-4)}
          </span>
        </div>
      </div>
              ...
              <div className="px-4 py-4 bg-zinc-50/50 dark:bg-zinc-800/20 space-y-4">
                ...
                {inputs.length > 0 && (
                  <div className="space-y-3">
                    {inputs.map((input, inputIndex: number) => (
                      <div key={inputIndex}>
                        ...
                        <input
                          ...
                          value={
                            functionInputs[funcKey]?.[input.name || `param${inputIndex}`] || ""
                          }
                          onChange={(e) =>
                            updateInput(funcKey, input.name || `param${inputIndex}`, e.target.value)
                          }
