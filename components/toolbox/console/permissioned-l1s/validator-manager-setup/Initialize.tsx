...
function Initialize({ onSuccess }: BaseConsoleToolProps) {
  ...
  return (
    <div>
      <Steps>
        <Step>
          ...
          <p className="text-sm text-gray-500">
            Select the proxy contract pointing to the ValidatorManager implementation you want to
            initialize.
          </p>
        ...
        <Step>
          ...
          <p className="text-sm text-gray-500">
            Enter the SubnetID of the Subnet/L1 this Validator Manager contract will manage the
            validators for. The P-Chain will only accept validator set changes from the Validator
            Manager contract addresses and blockchainID combination that was indicated in the
            ConvertSubnetToL1Tx.
          </p>
        ...
        <Step>
          ...
          <p className="text-sm text-gray-500">
            Set the intitial configuration for the Validator Manager contract. The admin address
            should be a multisig wallet for production L1s, since it can take full control over the
            L1 validator set by arbitrarily changing the validator set. The churn settings define
            how rapid changes to the validator set can be made.
          </p>
      ...
      {isInitialized !== null && (
        <p className="mt-4">
          Initialization Status: {isInitialized ? "Initialized" : "Not Initialized"}
        </p>
      )}
    </div>
  );
