...
export const useTestnetFaucet = () => {
  ...
  const claimEVMTokens = useCallback(
    async (chainId: number, silent: boolean = false): Promise<FaucetClaimResult> => {
      if (!walletEVMAddress) {
      ...
      if (!chainConfig) {
        throw new Error(`Unsupported chain or faucet not available for chain ID ${chainId}`);
      }
  ...
  const claimPChainAVAX = useCallback(
    async (silent: boolean = false): Promise<FaucetClaimResult> => {
      if (!pChainAddress) {
