...
export default function IntegrationsClient({ list }: IntegrationsClientProps) {
  ...
  const filteredIntegrations = useMemo(() => {
    ...
    return validIntegrations.filter((integration) => {
      const title =
        typeof integration.data.title === "string" ? integration.data.title.toLowerCase() : "";
      const description =
        typeof integration.data.description === "string"
          ? integration.data.description.toLowerCase()
          : "";
      const category =
        typeof integration.data.category === "string"
          ? integration.data.category.toLowerCase()
          : "";
      const available = Array.isArray(integration.data.available)
        ? integration.data.available
            .map((a: any) => (typeof a === "string" ? a.toLowerCase() : ""))
            .join(" ")
        : "";
  ...
  const integrations: { [category: string]: any[] } = {};
  ...
  const categories = Object.keys(integrations).sort((a, b) => {
    if (a === "Featured") {
      return -1;
    } else if (b === "Featured") {
      return 1;
            ...
            <div className="max-w-5xl mx-auto">
              <div className="flex flex-col lg:flex-row gap-4 items-start">
                ...
                <div className="relative flex-1">
                  ...
                  {mounted && searchQuery && (
                    <p
                      className="mt-3 text-sm text-slate-600 dark:text-slate-400 lg:text-left text-center"
                      suppressHydrationWarning
                    >
                      Found {filteredIntegrations.length} integration
              ...
              <aside className="w-full md:w-64 lg:w-72 shrink-0 order-first">
                <div className="md:sticky md:top-24">
                  <div className="backdrop-blur-sm bg-white/10 dark:bg-white/5 border border-slate-200/20 dark:border-white/20 shadow-sm rounded-xl p-5">
                    ...
                    <ul
                      className="space-y-1.5 max-h-[70vh] overflow-y-auto pr-2 
                      ...
                      {categories.map((category) => (
                        <li key={category} className="w-full">
                          ...
                          >
                            ...
                            <div
                              className="flex text-xs font-medium bg-slate-100 dark:bg-white/10 px-2 py-1 rounded-full text-slate-600 dark:text-slate-300 group-hover:bg-slate-200 dark:group-hover:bg-white/20 transition-colors duration-200 ml-2 shrink-0"
                              suppressHydrationWarning
                            >
                ...
                {categories.length === 0 && searchQuery && (
                  <div className="flex flex-col items-center justify-center py-16 px-4">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-center max-w-md">
                      We couldn&apos;t find any integrations matching &quot;{searchQuery}&quot;. Try
                        ...
                        >
                          ...
                          <div className="relative z-10 p-4 flex flex-col h-full gap-3">
                            ...
                            <div className="flex flex-col gap-4 mt-auto">
                              ...
                              {/* Available For Tags */}
                              {integration.data.available &&
                                integration.data.available.length > 0 && (
                                  <div className="flex flex-col gap-2">
                                    <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">
                                      Available For
                                    </p>
                                    <div className="flex flex-wrap gap-1.5">
                                      {integration.data.available.map(
