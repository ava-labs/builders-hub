...
The client does NOT submit a blockchain transaction. The authorization is gasless—the server or facilitator will submit it. For details on the `X-PAYMENT` header structure, see [HTTP Headers](./02-http-headers#x-payment-header).
...
![Detailed x402 payment flow](/tmpimagesx402/paymentflow2.png)
...
For comprehensive security details, see [Security Considerations](./05-security-considerations).
