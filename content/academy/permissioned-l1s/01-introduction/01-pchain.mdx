...
In the [L1 Creation Speedrun](/academy/permissioned-l1s/01-introduction/03-create-l1.mdx) section we will cover how to create an L1 using [Avalanche's Toolbox](https://build.avax.network/tools/l1-toolbox). To learn more about this architecture transition and the ACPs that enabled it, check out the [ETNA upgrade](/academy/etna-upgrade).
