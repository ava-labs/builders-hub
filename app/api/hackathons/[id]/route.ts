...
export async function PUT(req: NextRequest, context: any) {
  try {
    if (req.headers.get("x-api-key") !== env.APIKEY) throw new Error("Unauthorized");
    const { id } = await context.params;
    ...
    if (
      ...
      const updatedHackathon = await updateHackathon(
        id,
  ...
  try {
    ...
    if (updateData.hasOwnProperty("is_public") && typeof updateData.is_public === "boolean") {
      ...
      return NextResponse.json(
        { error: "Only is_public field can be updated via PATCH" },
        { status: 400 }
      );
    }
  } catch (error) {
