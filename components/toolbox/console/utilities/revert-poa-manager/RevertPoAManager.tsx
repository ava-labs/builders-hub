...
function RevertPoAManager({ onSuccess }: BaseConsoleToolProps) {
  ...
  return (
    <>
                ...
                <>
                  ...
                  {ownerType === 'EOA' && (
                    <p className="text-xs text-yellow-600 mt-1">
                      ⚠️ The ValidatorManager is owned by an EOA, not a PoAManager. This tool is for reverting from
                      PoAManager control.
                    </p>
                  )}
                  {ownerType === 'StakingManager' && (
                    <p className="text-xs text-yellow-600 mt-1">
                      ⚠️ The ValidatorManager is owned by a StakingManager contract. This tool is specifically for
                      PoAManager contracts.
                    </p>
                  )}
                </>
              )}
      ...
      {transferSuccess && <p className="mt-4 text-green-600">PoAManager revert initiated successfully!</p>}
    </>
  );
}
