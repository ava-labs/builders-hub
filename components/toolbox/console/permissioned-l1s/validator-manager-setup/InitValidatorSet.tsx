...
const add0x = (hex: string): `0x${string}` =>
...
function InitValidatorSet({ onSuccess }: BaseConsoleToolProps) {
  ...
  async function aggSigs() {
    ...
    const aggPromise = (async () => {
      const { message, justification, signingSubnetId } =
  ...
  const onInitialize = (debug: boolean = false) => {
    if (!conversionTxID) {
    ...
    <>
      <Steps>
        <Step>
          ...
          <p>
            Enter the P-Chain Transaction ID of the ConvertSubnetToL1Tx of the L1 this Validator
            Manager it is for. It is needed to fetch the conversion data containing the initial
            validator set. This validator set will be set up in the validator manager contract so
            the consensus weight of these validators can be changed or they can be removed entirely
            if desired.
          </p>
        ...
        <Step>
          ...
          With the aggregated signature, you can now initialize the Validator Manager contract
          state. This will set up the initial validator set and allow you to manage validators.
          <Input
