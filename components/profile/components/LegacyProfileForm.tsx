...
const firstTimeProfileSchema = baseProfileSchema.extend({
  notifications: z
    .boolean()
    .default(false)
    .refine((val) => val === true, {
...
export default function LegacyProfileForm({
  initialData,
  id,
}: {
  initialData: LegacyProfileFormProps;
  id: string;
}) {
  ...
  const schema = useMemo(
    () => (isFirstTime ? firstTimeProfileSchema : baseProfileSchema),
    [isFirstTime],
  );
  ...
  const onSkip = async () => {
    ...
    try {
      ...
      await axios.put(`/api/profile/${id}`, values).catch((error) => {
