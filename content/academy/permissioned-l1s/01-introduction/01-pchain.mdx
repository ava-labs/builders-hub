...
While technically one subnet can validate multiple chains (like the Primary Network validating P-Chain, C-Chain, and X-Chain), **in production it's almost always a 1:1 relationship**. We recommend deploying one chain per subnet for:
- **Simpler management**: Easier to reason about and operate
- **Resource isolation**: Each application gets dedicated validator resources  
- **Independent scaling**: Each chain can scale based on its own needs
...
- Validators sync and validate the entire Primary Network (X, P, C chains)
...
- Validators pay continuous fees (~1.33 AVAX/month) instead of staking
- Validators only sync the P-Chain, not the entire Primary Network
...
To learn more about this architecture transition and the ACPs that enabled it, check out the [ETNA upgrade](/academy/etna-upgrade).
