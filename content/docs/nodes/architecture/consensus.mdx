...
Avalanche consensus operates on a Directed Acyclic Graph (DAG) of transactions. It's used by the **X-Chain** for high-throughput asset transfers.
...
**Key Difference**: In Avalanche consensus, non-conflicting transactions can be processed in parallel, increasing throughput.
...
const (
    StateBootstrapping EngineState = iota
...
| State | Description |
|-------|-------------|
| **Bootstrapping** | Catching up with network state |
| **Normal Operation** | Participating in consensus |
