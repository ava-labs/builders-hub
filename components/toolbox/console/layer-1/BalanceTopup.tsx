...
const metadata: ConsoleToolMetadata = {
  ...
  description:
    "Increase the balance of a validator to extend its validation period and maintain network participation",
...
function ValidatorBalanceIncrease({ onSuccess }: BaseConsoleToolProps) {
  ...
  const [validatorSelection, setValidatorSelection] = useState<ValidationSelection>({
    validationId: "",
    nodeId: "",
  });
    ...
    try {
      ...
      setError(
        error instanceof Error
          ? error.message
          : "An unknown error occurred during the balance increase."
      );
    } finally {
      setLoading(false);
      setStatusMessage("");
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 space-y-2">
            ...
            {subnetId && (
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono text-blue-700 dark:text-blue-300 truncate max-w-[200px]">
                  {subnetId}
                </span>
              </div>
            )}
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            ...
            <Input
              ...
              button={
                <Button variant="secondary" className="pointer-events-none px-3" stickLeft>
                  AVAX
                </Button>
              }
            />
            ...
            <div className="space-y-2">
              ...
              {((error && error.toLowerCase().includes("balance")) ||
                (error && error.toLowerCase().includes("utxo"))) && (
          ...
          <Alert variant="error">
            This action will use AVAX from your P-Chain address (
            {pChainAddress
              ? `${pChainAddress.substring(0, 10)}...${pChainAddress.substring(pChainAddress.length - 4)}`
              : "Loading..."}
            ) to increase the balance of the specified L1 validator. Ensure the Validation ID is
            correct.
          </Alert>
          ...
          {error &&
            ...
            !error.toLowerCase().includes("utxo") &&
            !error.toLowerCase().includes("validation") &&
            !error.toLowerCase().includes("subnet") && <Alert variant="error">{error}</Alert>}
