...
- **Consensus engines**: Snowman (linear) and Avalanche (DAG) with Snowman++ proposer windows on Snowman chains.
...
- **Storage**: Pebble/LevelDB, shared atomic UTXO memory for cross-chain transfers, optional indexer.
...
| **Consensus Engines** | Snowman/Snowman++ (P/C + most L1s) and Avalanche DAG (X-Chain) |
...
| **Database** | Persistent storage using PebbleDB or LevelDB; shared atomic memory |
...
<Cards>
  ...
  <Card title="X-Chain (Exchange)" href="/docs/primary-network">
    High-throughput asset transfers using UTXO model. Uses AVM with Avalanche DAG.
  </Card>
</Cards>
