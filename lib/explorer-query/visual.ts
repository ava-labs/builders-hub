...
- Two to four headline stats across the top: the total, the peak or the share that answers the question, the reverted share when reverts matter. Use agg over a column of the rows (sum for counts and fees, max for peaks, avg for rates, distinct for how many groups, first or last for the newest or oldest bucket). Give each a sub line that says what it is in four words or fewer.
- Callouts: one to three sentences that read the actual numbers in the rows: the top item and its share, an anomaly (every call of a method reverted; one sender behind a whole method), a partial last bucket. State figures, not adjectives. No em dashes. Never say "settled" or "waiting".
