...
export function generateGenesis({
  ...
  const allocations: Record<
    string,
    { balance: string; code?: string; storage?: Record<string, string>; nonce?: string }
  > = {};
  ...
  if (preinstallConfig?.proxy) {
    allocations[PROXY_ADDRESS.slice(2).toLowerCase()] = {
      ...
      storage: {
        "0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc": hexTo32Bytes(
          UNITIALIZED_PROXY_ADDRESS.slice(2).toLowerCase()
        ),
        "0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103": hexTo32Bytes(
          PROXY_ADMIN_ADDRESS.slice(2).toLowerCase()
        ),
      },
      nonce: "0x1",
    };
  ...
  if (preinstallConfig?.wrappedNativeToken) {
    ...
    const nameHex =
      "0x" +
      Buffer.from(wrappedTokenName, "utf8").toString("hex").padEnd(62, "0") +
      (wrappedTokenName.length * 2).toString(16).padStart(2, "0");

    // Encode symbol: e.g. "WAVAX" (dynamic length)
    // Format: [data][length*2] in hex
    const symbolHex =
      "0x" +
      Buffer.from(wrappedTokenSymbol, "utf8").toString("hex").padEnd(62, "0") +
      (wrappedTokenSymbol.length * 2).toString(16).padStart(2, "0");
