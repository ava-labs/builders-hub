...
export async function getSubnetInfo(subnetId: string, signal?: AbortSignal): Promise<SubnetInfo> {
  ...
  try {
    ...
    return getSubnetInfoForNetwork(otherNetwork, subnetId, signal);
  }
}
...
export async function getChainDetails(chainId: string): Promise<ChainDetails> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch chain details for chainId ${chainId}: ${response.status} ${response.statusText}`
    );
  }
