...
function CrossChainTransfer({ suggestedAmount = "0.0", onSuccess }: CrossChainTransferProps) {
  ...
  const onBalanceChanged = useCallback(async () => {
    try {
      ...
      setCriticalError(
        new Error(`Failed to update balances: ${e instanceof Error ? e.message : String(e)}`)
      );
    }
  ...
  const pollForUTXOChanges = useCallback(async () => {
    try {
      ...
      setCriticalError(
        new Error(`Failed to fetch UTXOs: ${e instanceof Error ? e.message : String(e)}`)
      );
    }
  }, [fetchUTXOs]);
  ...
  const handleMaxAmount = () => {
    const maxAmount =
      sourceChain === "c-chain" ? cChainBalance.toString() : pChainBalance.toString();
    setAmount(maxAmount);
  };
  ...
  const totalUtxoAmount =
    destinationChain === "p-chain" ? totalCToPUtxoAmount : totalPToCUtxoAmount;
  ...
  useEffect(() => {
    ...
    setStep2Expanded(
      step2Status === "active" || step2Status === "error" || step2Status === "completed"
    );
  }, [
    exportLoading,
    completedExportTxId,
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="font-medium">
                    Avalanche {sourceChain === "c-chain" ? "C-Chain" : "P-Chain"}
                  </div>
        ...
        <AmountInput
          ...
          button={
            <Button
              ...
              disabled={
                exportLoading ||
                (sourceChain === "c-chain" ? cChainBalance <= 0 : pChainBalance <= 0)
              }
              stickLeft
            >
              MAX
          ...
          <div className="relative">
            <div className="w-full flex items-center justify-between px-4 py-3 bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 rounded-lg text-zinc-600 dark:text-zinc-400 cursor-not-allowed opacity-75">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="font-medium">
                    Avalanche {destinationChain === "c-chain" ? "C-Chain" : "P-Chain"}
                  </div>
        ...
        {availableUTXOs.length > 0 && !importTxId && (
          <>
            <div className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">
              {totalUtxoAmount.toFixed(6)} AVAX available to import to{" "}
              {destinationChain === "p-chain" ? "P-Chain" : "C-Chain"}
            </div>
            ...
            <div className="space-y-2 mb-4">
              {availableUTXOs.map((utxo, index) => (
                <div
                  key={index}
                  className="text-sm font-mono text-zinc-700 dark:text-zinc-200 bg-zinc-100 dark:bg-zinc-800 p-3 rounded border border-zinc-200 dark:border-zinc-700"
                >
