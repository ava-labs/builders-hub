...
This post walks the complete flow with working code: derive the P-Chain address, move AVAX from the C-Chain via atomic transfer (export + import), and delegate to a validator. Everything you need is inline — the only dependencies are [`@avalabs/avalanchejs`](https://github.com/ava-labs/avalanchejs), the [`@fireblocks/ts-sdk`](https://developers.fireblocks.com/docs/getting-started-with-the-fireblocks-sdks), and `@noble/hashes`.
