...
const RemoveExpiredValidatorRegistration: React.FC = () => {
  ...
  const initiatedEventAbi = useMemo(() => {
    ...
    return abi.find((i) => i.type === "event" && i.name === "InitiatedValidatorRegistration") as
      | AbiEvent
      | undefined;
  }, []);
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      console.log(
        `Searching ${totalBlocks} blocks (${totalChunks} chunks) from block ${startBlock} to ${latest}`
      );
    ...
    const run = async () => {
      try {
        ...
        const uniqueIds = Array.from(
          new Set(events.map((e) => (e.validationId || "").toLowerCase()).filter(Boolean))
        );
  ...
  const handleForceRemove = async (validationId: string) => {
    ...
    try {
      ...
      if (!coreWalletClient || !viemChain || !coreWalletClient.account) {
        throw new Error("Wallet/chain not initialized");
      }
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div className="text-xs text-zinc-600 dark:text-zinc-400">
              {fromBlock && (
                <span>
                  Will search from block <span className="font-mono font-medium">{fromBlock}</span>{" "}
                  to latest
                </span>
              )}
            </div>
        ...
        {events.length > 0 && (
          <div className="flex items-center justify-between text-[11px] text-zinc-600 dark:text-zinc-300">
            <div>
              ...
              of{" "}
              <span className="font-medium text-zinc-800 dark:text-zinc-100">{events.length}</span>{" "}
              events (expired).
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-lg p-4 border border-zinc-200 dark:border-zinc-800 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-base font-semibold text-zinc-800 dark:text-white">
                InitiatedValidatorRegistration Events
              </h3>
              ...
              <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                ...
                <tbody className="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-700">
                  {filteredEvents.map((ev, idx) => (
                    ...
                    >
                      <td className="px-3 py-3 text-xs font-mono break-all text-zinc-800 dark:text-zinc-200">
                        {ev.validationId}
                      </td>
                        ...
                        {(() => {
                          ...
                          return url ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="font-mono break-all text-blue-700 dark:text-blue-400 hover:underline"
                            >
                              {ev.txHash}
                            </a>
                          ) : (
                      ...
                      <td className="px-3 py-3 text-[11px] text-zinc-800 dark:text-zinc-200">
                        {(() => {
                          ...
                          return (
                            <span
                              className={`inline-flex items-center rounded-full px-2 py-0.5 border ${statusBadgeClass(s)}`}
                            >
                              {statusLabel(s)}
                            </span>
                          );
                      ...
                      <td className="px-3 py-3 text-xs text-zinc-800 dark:text-zinc-200">
                        <div className="flex items-center justify-end">
                          ...
                          >
                            {actionState[ev.validationId]?.isProcessing
                              ? "Processing…"
                              : "Force Remove"}
                          </Button>
                        </div>
                        ...
                        {actionState[ev.validationId]?.evmTxHash && (
                          <div className="mt-1 text-[10px] break-all text-zinc-600 dark:text-zinc-300">
                            {(() => {
                              const txh = actionState[ev.validationId]?.evmTxHash as unknown as
                                | string
                                | undefined;
        ...
        {!isLoading && !error && events.length === 0 && validatorManagerAddress && (
          <div className="p-3 rounded-md bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 text-sm">
            ...
            <p className="text-xs mt-1">
              Try searching a larger block range by entering an earlier "From Block" number above,
              or leave it empty to search the last 100,000 blocks. You can also enter "0" to search
