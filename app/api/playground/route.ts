    ...
    if (playgroundId) {
      ...
      const playground = await prisma.statsPlayground.findFirst({
        where: {
          id: playgroundId,
          OR: session?.user
            ? [{ user_id: session.user.id }, { is_public: true }]
            : [{ is_public: true }],
        },
        include: {
      ...
      const isFavorited =
        session?.user && playground.favorites ? playground.favorites.length > 0 : false;
      ...
      const globalStartTime = Array.isArray(chartsData)
        ? null
        : chartsData?.globalStartTime || null;
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (charts !== undefined || globalStartTime !== undefined || globalEndTime !== undefined) {
      ...
      const chartsArray = Array.isArray(existingCharts)
        ? existingCharts
      ...
      updateData.charts = {
        globalStartTime:
          globalStartTime !== undefined
            ...
            : existingCharts?.globalStartTime || null,
        globalEndTime:
          globalEndTime !== undefined
            ? globalEndTime || null
            : existingCharts?.globalEndTime || null,
