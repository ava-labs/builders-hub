...
export default function DeployRegistrarStep() {
  const { verifiers, registrarAddress, setRegistrarAddress, setLastTxHash, setGlobalError } = useEERCDeployStore();
  const { deploy, isDeploying } = useContractDeployer();

  const canDeploy = verifiers.registration.length > 0;
  const done = registrarAddress.length > 0;
  ...
  return (
    <ContractDeployViewer contracts={REGISTRAR_SOURCES}>
        ...
        <div className="flex-1 overflow-auto p-5 space-y-4">
          {!canDeploy && (
            <div className="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/10 p-3 text-xs text-amber-700 dark:text-amber-300">
              Registration verifier must be deployed first — go back one step.
            </div>
          )}
          <div
            className={
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Deploy Registrar</h3>
                <p className="mt-1.5 text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
                  Per-chain singleton that maps EVM addresses to BabyJubJub public keys. Takes the registration verifier
                  (deployed in step 3) as its constructor argument. Every eERC operation reads from here to encrypt
                  amounts to the right counterparty.
                </p>
