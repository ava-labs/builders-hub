...
export default function NetworkDiagram({
  ...
  const supportsFullscreen =
    ...
    (document.fullscreenEnabled ||
      ...
      (document as FullscreenDocument).mozFullScreenEnabled ||
      (document as FullscreenDocument).msFullscreenEnabled);
  ...
  const toggleFullscreen = useCallback(() => {
    ...
    const fsContainer = container as FullscreenElement;
  ...
  const initializeLayout = useCallback(
    (width: number, height: number, chains: ChainCosmosData[]): ChainNode[] => {
      ...
      sortedChains.forEach((chain, index) => {
        ...
        const validatorRadius = Math.max(
          0.8,
          Math.min(2.5, Math.sqrt(areaPerValidator / Math.PI) * 0.6)
        );
        ...
        while (placed < targetValidators) {
          ...
          const validatorsInRing =
            ring === 0 ? 1 : Math.floor(circumference / (validatorRadius * 2.5));
  ...
  const calculateICMLinks = useCallback(
    (nodes: ChainNode[], flows: ICMFlowRoute[]): ChainLink[] => {
  ...
  const initializeParticles = useCallback(
    (links: ChainLink[], maxMessages: number): MessageParticle[] => {
  ...
  const simulatePhysics = useCallback(
    (nodes: ChainNode[], width: number, height: number, allowOverflow: boolean) => {
      const centerX = width / 2;
      const centerY = height / 2;
      const padding = 60;
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (!allowOverflow) {
          nodeA.x = Math.max(
            nodeA.radius + padding,
            Math.min(width - nodeA.radius - padding, nodeA.x)
          );
          nodeA.y = Math.max(
            nodeA.radius + padding,
            Math.min(height - nodeA.radius - padding, nodeA.y)
          );
        }
      }
    },
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      particlesRef.current.forEach((particle) => {
        ...
        const pos = getPointOnLine(particle.progress, link.fromX, link.fromY, link.toX, link.toY);
      ...
      nodes.forEach((node) => {
        ...
        if ((node.tps || 0) > 0) {
          ...
          for (let w = 0; w < numWaves; w++) {
            ...
            const waveRadius =
              node.radius * scale +
              waveProgress * (maxDistance + tpsRatio * (maxDistance - minDistance));
      ...
      {(selectedChain || hoveredChain) &&
        (() => {
          ...
          const getChainSlug = (chainId?: string, chainName?: string): string | null => {
            ...
            const chain = (l1ChainsData as L1ChainData[]).find(
          ...
          return (
            <div className="absolute bottom-4 right-4 z-50 pointer-events-auto max-h-[calc(100vh-120px)] overflow-y-auto">
                ...
                <div className="flex items-center gap-3 mb-3 pr-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-white text-lg">{displayChain.name}</h3>
                      {displayChain.category && (
                        <span
                          className={`text-[10px] px-1.5 py-0.5 rounded-full border ${getCategoryColor(displayChain.category)}`}
                        >
                          {displayChain.category}
                        </span>
                      )}
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div className="bg-white/5 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-white">
                      {formatNum(displayChain.activeAddresses)}
                    </p>
                    <p className="text-[10px] text-white/50">Active Addrs</p>
                  </div>
                  <div className="bg-white/5 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-white">
                      {formatNum(displayChain.txCount)}
                    </p>
                  ...
                  <div className="bg-white/5 rounded-lg p-2 text-center">
                    {displayChain.chainId && failedChainIds.includes(displayChain.chainId) ? (
                      ...
                      <p className="text-lg font-bold text-cyan-400">
                        {formatNum(displayChain.icmMessages)}
                      </p>
                    )}
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div className="bg-white/5 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-purple-400">
                      {displayChain.validatorCount.toLocaleString()}
                    </p>
                    <p className="text-[10px] text-white/50">Validators</p>
                  </div>
                  <div className="bg-white/5 rounded-lg p-2 text-center">
                    <p className="text-lg font-bold text-green-400">
                      {displayChain.tps !== undefined ? displayChain.tps : "N/A"}
                    </p>
                  ...
                  <div className="bg-white/5 rounded-lg p-2 text-center">
                    {displayChain.chainId && failedChainIds.includes(displayChain.chainId) ? (
                      ...
                      <p className="text-lg font-bold text-amber-400">
                        {formatNum(totalOutgoing + totalIncoming)}
                      </p>
                    )}
