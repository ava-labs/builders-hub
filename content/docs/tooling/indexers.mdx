...
[Space and Time](https://spaceandtime.io) is a decentralized data warehouse that provides blockchain indexing services for major blockchains like Bitcoin, Ethereum, Avalanche, Base, etc., offering developers a way to query ZK-proven blockchain data is a trustless way.

Easily load in your own data from source systems while we encrypt data in-database, in-transit, and in-use, and enable developers to join public onchain data with sensitive, private offchain data they load in.

For more details, please refer to our documentation [Space and Time documentation](https://docs.spaceandtime.io)
