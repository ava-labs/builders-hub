...
export function useValidatorManagerDetails({
  subnetId,
}: UseValidatorManagerDetailsProps): ValidatorManagerDetails {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        console.log(
          `Fetching Validator Manager details for subnet: ${subnetId} on network: ${network}`
        );
        ...
        if (viemChain && viemChain.id !== expectedChainIdForVMC) {
          setError(
            `Please use chain ID ${expectedChainIdForVMC} in your wallet. Current selected chain ID: ${viemChain.id}`
          );
          setIsLoading(false);
          return;
        }
        ...
        if (connectedChainId !== expectedChainIdForVMC) {
          setError(
            `Please connect to chain ID ${expectedChainIdForVMC} to use this L1's Validator Manager. Connected: ${connectedChainId}`
          );
          setIsLoading(false);
          return;
        }
        ...
        const blockchainInfoForSigning = await getBlockchainInfoForNetwork(
          network,
          vmcBlockchainId
        );
    ...
    fetchDetails();
  }, [subnetId, getChainIdFn, viemChain?.id, avalancheNetworkID, publicClient]);
  ...
  useEffect(() => {
    const fetchL1TotalWeight = async () => {
      ...
      if (!validatorManagerAddress) {
        // If no VMC address yet, don't attempt to fetch
        setContractTotalWeight(0n);
        setL1WeightError(null);
        setIsLoadingL1Weight(false);
        return;
      ...
      try {
        ...
        if (
          ...
          setL1WeightError(
            "Failed to load L1 weight data from contract. Check network or VMC address."
          );
        }
      } finally {
        setIsLoadingL1Weight(false);
