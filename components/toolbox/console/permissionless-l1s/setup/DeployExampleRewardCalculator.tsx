  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500">
        This will deploy the <code>ExampleRewardCalculator</code> contract to the EVM network{" "}
        <code>{viemChain?.id}</code>. The Example Reward Calculator implements a linear,
        non-compounding reward calculation that rewards a set percentage of tokens per year.
      </p>
      <p className="text-sm text-gray-500">
        Contract source:{" "}
        <a href={EXAMPLE_REWARD_CALCULATOR_SOURCE_URL} target="_blank" rel="noreferrer">
      ...
      <p>
        Deployment Status: <code>{rewardCalculatorAddress || "Not deployed"}</code>
      </p>

      {rewardCalculatorAddress && (
        <Success label="Example Reward Calculator Address" value={rewardCalculatorAddress} />
      )}
    </div>
  );
}
