...
export const generateChainConfig = (
  ...
  const addIfNotDefault = (key: string, value: any, defaultValue?: any) => {
    const defaultVal =
      defaultValue !== undefined ? defaultValue : SUBNET_EVM_DEFAULTS[key as keyof typeof SUBNET_EVM_DEFAULTS];
 ...
 */
export const encodeChainConfig = (blockchainId: string, chainConfig: any) => {
 ...
 */
export const generateConfigFileCommand = (blockchainId: string, chainConfig: any) => {
 ...
 */
export const generatePrimaryNetworkDockerCommand = (nodeType: 'validator' | 'rpc' | 'archival', networkID: number) => {
  const isRPC = nodeType === 'rpc' || nodeType === 'archival';
  const isTestnet = networkID === 5;
