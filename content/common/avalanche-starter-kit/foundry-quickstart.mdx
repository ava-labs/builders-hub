...
```toml
[profile.default]
evm_version = "cancun"  # Required for Avalanche (Pectra not yet supported)
...
> **EVM Version Required**: The `evm_version = "cancun"` setting is required when deploying to Avalanche. Solidity v0.8.30+ defaults to Pectra, which uses opcodes not yet supported on Avalanche. Without this setting, your contracts may fail to deploy or behave unexpectedly.
