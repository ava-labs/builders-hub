...
The `AllowList` interface is used by precompiles to check if a given address is allowed to use a precompiled contract. `AllowList` consist of three roles, `Admin`, `Manager` and `Enabled`. `Admin` can add/remove addresses of any role, including other `Admin` and `Manager` addresses. `Manager` is introduced with Durango upgrade and can add/remove `Enabled` addresses, without the ability to add/remove `Admin` or `Manager` addresses. `Enabled` addresses can use the precompiled contract, but cannot modify other roles.
...
In this example, `0x8db97C7cEcE249c2b98bDC0226Cc4C2A57BF52FC` is named as the `Admin` of the `ContractDeployerAllowList`. This enables it to add other `Admin`, `Manager` or to add `Enabled` addresses. `Admin`, `Manager` and `Enabled` can all deploy contracts. To provide a great UX with factory contracts, the `tx.Origin` is checked for being a valid deployer instead of the caller of `CREATE`. This means that factory contracts will still be able to create new contracts as long as the sender of the original transaction is an allow listed deployer.
...
- If you attempt to add an `Enabled` or `Manager` and you are not an `Admin`, you will see something like: ![admin fail](/images/customize1.png)
- If you attempt to deploy a contract but you are not an `Admin` not a `Manager` and not an `Enabled`, you will see something like: ![deploy fail](/images/customize2.png)
- If you call `readAllowList(addr)` then you can read the current role of `addr`, which will return a uint256 with a value of 0, 1, 2 or 3, corresponding to the roles `None`, `Enabled`, `Admin` and `Manager` respectively.
...
- If you attempt to add an `Enabled` or `Manager` and you are not an `Admin`, you will see something like: ![admin fail](/images/customize3.png)
...
`adminAddresses` denotes admin accounts who can add other `Admin`, `Manager` or `Enabled` accounts. `Admin`, `Manager` and `Enabled` are all eligible to mint native coins for other addresses. `ContractNativeMinter` uses same methods as in `ContractDeployerAllowList`.
...
Each precompile lives in its own package under `/precompile/contracts`, containing the Go implementation together with the Solidity interface and ABI that contracts use to call it. Precompiles built on the allow list, such as `txallowlist` and `deployerallowlist`, share the common interface in [/precompile/allowlist](https://github.com/ava-labs/avalanchego/tree/master/graft/subnet-evm/precompile/allowlist). See [the precompile packages](https://github.com/ava-labs/avalanchego/tree/master/graft/subnet-evm/precompile/contracts) for reference implementations.
...
These upgrades must be specified in a file named `upgrade.json` placed in the same directory where [`config.json`](#avalanchego-chain-configs) resides: `{chain-config-dir}/{blockchainID}/upgrade.json`. For example, the `Dispatch L1` on Fuji would be `~/.avalanchego/configs/chains/2D8RG4UpSXbPbvPCAWppNJyqTG2i2CAXSkTgmTBBvs7GKNZjsY/upgrade.json`.
...
```bash
INFO [08-15|15:09:36.772] <2D8RG4UpSXbPbvPCAWppNJyqTG2i2CAXSkTgmTBBvs7GKNZjsY Chain>
github.com/ava-labs/subnet-evm/eth/backend.go:155: Initialised chain configuration
config=“{ChainID: 779672 Homestead: 0 EIP150: 0 EIP155: 0 EIP158: 0 Byzantium: 0
...
Notice that `precompileUpgrades` entry correctly reflects the changes. You can also check the activated precompiles at a timestamp with the [`eth_getActiveRulesAt`](/docs/rpcs/subnet-evm#eth_getActiveRulesAt) RPC method. The [`eth_getChainConfig`](/docs/rpcs/subnet-evm#eth_getchainconfig) RPC method will also return the configured upgrades in the response.
...
<Callout title="Note">
It's still possible to add `adminAddresses`, `managerAddresses` or `enabledAddresses` along with these initial configurations. In this case, the precompile will be activated with the initial configuration, and admin, manager or enabled addresses can access to the precompiled contract normally.
</Callout>
...
As described in [this doc](/docs/nodes/configure/configs-flags#avalanche-l1-chain-configs), each blockchain of Avalanche L1s can have its own custom configuration. If an Avalanche L1's ChainID is `2D8RG4UpSXbPbvPCAWppNJyqTG2i2CAXSkTgmTBBvs7GKNZjsY`, the config file for this chain is located at `{chain-config-dir}/2D8RG4UpSXbPbvPCAWppNJyqTG2i2CAXSkTgmTBBvs7GKNZjsY/config.json`.
