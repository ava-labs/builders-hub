...
function buildL1CreationGuidance(): string {
  return [
    ...
    '   - `platform-cli subnet create --key-name <key> --network <fuji|mainnet>`',
    '   - `platform-cli chain create --subnet-id <id> --name <name> --genesis genesis.json`',
    '   - `platform-cli subnet convert-to-l1 --subnet-id <id> --chain-id <id> --manager <addr> --validators <node>`',
