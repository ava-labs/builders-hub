...
In the [Subnet Creation](/academy/permissioned-l1s/01-introduction/03-interop) section we will cover how to create an Subnet using [Avalanche's Developer Console](https://build.avax.network/tools/l1-toolbox). To learn more about this architecture transition and the ACPs that enabled it, check out the [ETNA upgrade](/academy/avalanche-fundamentals/03-multi-chain-architecture-intro/03a-etna-upgrade).