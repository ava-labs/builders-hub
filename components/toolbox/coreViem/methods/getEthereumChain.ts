...
export type GetEthereumChainResponse = Extract<
  CoreWalletRpcSchema[number],
...
export async function getEthereumChain(client: AvalancheWalletClient): Promise<GetEthereumChainResponse> {
  const chain = await client.request<Extract<CoreWalletRpcSchema[number], { Method: 'wallet_getEthereumChain' }>>({
    method: 'wallet_getEthereumChain',
    params: [],
  });
  return chain;
