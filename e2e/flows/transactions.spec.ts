...
// Gated on QA_WALLET_KEY: present → run the real Fuji tx flows; absent → skip.
// The workflow only passes the key on PRODUCTION deploys (and manual dispatch),
// so preview/PR runs skip the flows and run the deterministic render gate only.
// Locally, export QA_WALLET_KEY to run them.
