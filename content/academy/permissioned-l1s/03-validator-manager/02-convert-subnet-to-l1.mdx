...
Now that you understand where to deploy your Validator Manager Contract (VMC), and you [created a Subnet](/academy/permissioned-l1s/01-introduction/03-create-subnet) with at least one validator node, it's time to convert your Subnet into a sovereign L1. 
...
3. **Validator Manager Address**: The address of the validator manager contract (or proxy) that will manage the L1's validator set. Chains created with Toolbox have a pre-deployed proxy at `0xfacade0000000000000000000000000000000000`. In the [next section](/academy/permissioned-l1s/03-validator-manager/03-deploy-proxy-contract) we will further explain the reason behind this Proxy.

4. **Initial Validators**: The validators that will form your L1's initial validator set.
