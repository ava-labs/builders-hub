...
|------|-------------|---------|
| `--network-dir` | Path to target network directory | `$TMPNET_NETWORK_DIR` or error |
...
| `--plugin-dir` | string | Directory containing VM plugins | `$AVALANCHE_PLUGIN_DIR` or `~/.avalanchego/plugins` |
...
tmpnetctl start-network --avalanchego-path=./build/avalanchego

# Start 3-node network
tmpnetctl start-network --avalanchego-path=./build/avalanchego --node-count=3
