...
```bash
forge create --rpc-url dispatch-l1 --private-key $PK contracts/interchain-messaging/invoking-functions/CalculatorReceiverOnDispatch.sol:CalculatorReceiverOnDispatch --broadcast
```
```
[⠊] Compiling...
...
```bash
cast call --rpc-url dispatch-l1 $RECEIVER_ADDRESS "result_num()(uint)"
```
