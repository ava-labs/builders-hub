...
Avalanche node configurations are loaded from specific paths. For chain-specific configurations, use the following structure:

```
<chain-config-dir>/<blockchainID>/config.json
```

By default `<chain-config-dir>` is `~/.avalanchego/configs/chains/`. If you specify a custom directory with `--chain-config-dir`, remember that it should point to the parent directory of the chains folder, not include the "chains" part:

```bash
# CORRECT
--chain-config-dir=/etc/.avalanchego/configs

# INCORRECT
--chain-config-dir=/etc/.avalanchego/configs/chains
```

If you want to use a different data directory overall, use the `--data-dir` flag:

```bash
--data-dir=/etc/.avalanchego/
```
...
For example, if configuring the C-Chain, place this in:
```
~/.avalanchego/configs/chains/C/config.json
```

Or for a custom Avalanche L1 chain with ID "2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt":
```
~/.avalanchego/configs/chains/2ebCneCbwthjQ1rYT41nhd7M76Hc6YmosMAQrTFhBq8qeqh6tt/config.json
```

You can verify pruning is disabled by checking the node's startup logs. Look for `Pruning:false` in the VM configuration.
...
#### Transaction History and Lookup Limits
...
When switching between database types (e.g., from LevelDB to PebbleDB), you should start synchronization from genesis to ensure proper historical data. To use PebbleDB:

```bash
--db-type=pebbledb
```

If switching database types, do not reuse the old database directory. Start with a fresh sync.

#### Comprehensive Archival Node Configuration Example

Here's a complete chain configuration for an archival C-Chain node:
