...
function GenesisBuilderInner({
  ...
  const [evmChainId, setEvmChainId] = useState<number>(
    MIN_CHAIN_ID + Math.floor(Math.random() * (MAX_CHAIN_ID - MIN_CHAIN_ID))
  );
  ...
  useEffect(() => {
    if (
      ...
      setTokenAllocations([
        { address: walletEVMAddress as Address, amount: tokenAllocations[0].amount },
      ]);
    }
  ...
  const [contractDeployerAllowListConfig, setContractDeployerAllowListConfig] =
    useState<AllowlistPrecompileConfig>(generateEmptyAllowlistPrecompileConfig());
  const [contractNativeMinterConfig, setContractNativeMinterConfig] =
    useState<AllowlistPrecompileConfig>(generateEmptyAllowlistPrecompileConfig());
  const [txAllowListConfig, setTxAllowListConfig] = useState<AllowlistPrecompileConfig>(
    generateEmptyAllowlistPrecompileConfig()
  );
  const [feeManagerConfig, setFeeManagerConfig] = useState<AllowlistPrecompileConfig>(
    generateEmptyAllowlistPrecompileConfig()
  );
  const [rewardManagerConfig, setRewardManagerConfig] = useState<AllowlistPrecompileConfig>(
    generateEmptyAllowlistPrecompileConfig()
  );
  ...
  useEffect(() => {
    ...
    if (!evmChainId || evmChainId < 1) errors.chainId = "Chain ID must be a positive integer";
      ...
      [
        1, 43113, 43114, 53935, 73772, 78430, 78431, 78432, 78433, 78434, 78435, 78436, 78437,
        78438, 78439,
      ].includes(evmChainId)
    ) {
    ...
    else if (!/^[a-zA-Z0-9\s]+$/.test(tokenName)) {
      errors.tokenName = "Token name can only contain letters, numbers, and spaces";
    }

    if (!tokenSymbol || tokenSymbol.length === 0) errors.tokenSymbol = "Token symbol is required";
    else if (tokenSymbol.length > 10) {
      errors.tokenSymbol = "Token symbol must be 10 characters or less";
    } else if (!/^[A-Z0-9]+$/.test(tokenSymbol)) {
      errors.tokenSymbol = "Token symbol must be uppercase letters and numbers only";
    }

    // Gas Limit
    if (gasLimit < MIN_GAS_LIMIT) {
      errors.gasLimit = `Gas limit must be at least ${MIN_GAS_LIMIT.toLocaleString()}`;
    } else if (gasLimit > MAX_GAS_LIMIT) {
      warnings.gasLimit = "High gas limits may impact performance";
    } else if (gasLimit < RECOMMENDED_MIN_GAS_LIMIT) {
      warnings.gasLimit = `Gas limit below ${RECOMMENDED_MIN_GAS_LIMIT.toLocaleString()} may be too restrictive`;
    }

    // Target Block Rate
    if (targetBlockRate < 0.1) {
      errors.targetBlockRate = "Target block rate must be at least 0.1 seconds";
    } else if (targetBlockRate > 10) {
      warnings.targetBlockRate = "Block rates above 10 seconds may impact user experience";
    }

    // Token Allocations
    if (tokenAllocations.length === 0) {
      errors.tokenAllocations = "At least one token allocation is required";
    }
    ...
    if (!isValidAllowlistPrecompileConfig(contractDeployerAllowListConfig)) {
      errors.contractDeployerAllowList =
        "Contract Deployer Allow List: Configuration is invalid or requires at least one valid address.";
    }
    if (!isValidAllowlistPrecompileConfig(contractNativeMinterConfig)) {
      errors.contractNativeMinter =
        "Native Minter: Configuration is invalid or requires at least one valid address.";
    }
    if (!isValidAllowlistPrecompileConfig(txAllowListConfig)) {
      errors.txAllowList =
        "Transaction Allow List: Configuration is invalid or requires at least one valid address.";
    }

    // Fee/Reward Manager
    if (!isValidAllowlistPrecompileConfig(feeManagerConfig)) {
      errors.feeManager =
        "Fee Manager: Configuration is invalid or requires at least one valid address.";
    }
    if (!isValidAllowlistPrecompileConfig(rewardManagerConfig)) {
      errors.rewardManager =
        "Reward Manager: Configuration is invalid or requires at least one valid address.";
    }

    // Fee Config Parameters
    if (feeConfig.minBaseFee < 0) errors.minBaseFee = "Min base fee must be non-negative";
    if (feeConfig.minBaseFee < gweiToWei(1)) {
      warnings.minBaseFee = "Min base fee below 1 gwei may cause issues";
    }
    if (feeConfig.minBaseFee > gweiToWei(500)) {
      warnings.minBaseFee = "Min base fee above 500 gwei may be expensive";
    }

    if (feeConfig.targetGas < 0) errors.targetGas = "Target gas must be non-negative";
    if (feeConfig.targetGas < MIN_TARGET_GAS) {
      warnings.targetGas = "Target gas below 1M may lead to congestion";
    }
    ...
    if (feeConfig.targetGas > MAX_TARGET_GAS) {
      ...
      feeConfig.targetGas > staticGasThreshold &&
      feeConfig.targetGas < staticGasThreshold * 1.5
    ) {
      // Info message when in static pricing range
      warnings.targetGas =
        "Target gas configured for static pricing (no congestion-based adjustments)";
    }

    if (feeConfig.baseFeeChangeDenominator < 0) {
      errors.baseFeeChangeDenominator = "Base fee change denominator must be non-negative";
    }
    if (feeConfig.baseFeeChangeDenominator < 8) {
      warnings.baseFeeChangeDenominator = "Low denominator may cause fees to change too rapidly";
    }
    if (feeConfig.baseFeeChangeDenominator > 1000) {
      warnings.baseFeeChangeDenominator = "High denominator may cause fees to react too slowly";
    }

    if (feeConfig.minBlockGasCost < 0) {
      errors.minBlockGasCost = "Min block gas cost must be non-negative";
    }
    if (feeConfig.minBlockGasCost > 1e9) {
      warnings.minBlockGasCost = "Min block gas cost above 1B may impact performance";
    }

    if (feeConfig.maxBlockGasCost < feeConfig.minBlockGasCost) {
      errors.maxBlockGasCost = "Max block gas cost must be >= min block gas cost";
    }
    if (feeConfig.maxBlockGasCost > 1e10) {
      warnings.maxBlockGasCost = "Max block gas cost above 10B may impact performance";
    }

    if (feeConfig.blockGasCostStep < 0) {
      errors.blockGasCostStep = "Block gas cost step must be non-negative";
    }
    if (feeConfig.blockGasCostStep > 5000000) {
      warnings.blockGasCostStep =
        "Block gas cost step above 5M may cause fees to change too rapidly";
    }
  ...
  const generateGenesisData = useCallback(() => {
    ...
    try {
      ...
      setGenesisData(
        `Error generating genesis: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }, [
  ...
  const isSectionExpanded = useCallback(
    (sectionId: SectionId) => expandedSections.has(sectionId),
    [expandedSections]
  );
  ...
  const handleTokenAllocationsChange = useCallback(
    (newAllocations: AllocationEntry[]) => {
  ...
  const handleDeployerConfigChange = useCallback(
    (config: SetStateAction<AllowlistPrecompileConfig>) => {
  ...
  const handleNativeMinterConfigChange = useCallback(
    (config: SetStateAction<AllowlistPrecompileConfig>) => {
  ...
  const handleSetFeeManagerConfig = useCallback(
    (config: SetStateAction<AllowlistPrecompileConfig>) => {
  ...
  const handleSetRewardManagerConfig = useCallback(
    (config: SetStateAction<AllowlistPrecompileConfig>) => {
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs">
                <div className="space-y-2">
                  <p className="text-xs">
                    A unique identifier for your blockchain network. Choose an ID that doesn't
                    conflict with existing chains.
                  </p>
                  <a
