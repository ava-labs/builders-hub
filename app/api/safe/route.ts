...
const getSupportedChain = async (
  chainId: string
): Promise<{ txServiceUrl: string; shortName: string }> => {
  try {
  ...
  try {
    ...
    switch (action) {
      ...
      case "getTransaction": {
        ...
        if (!safeTxHash) {
          return NextResponse.json({ error: "Missing safeTxHash" }, { status: 400 });
        }
      ...
      case "getAllSafesInfo": {
        ...
        if (!safeAddresses || !Array.isArray(safeAddresses)) {
          return NextResponse.json({ error: "Missing safeAddresses array" }, { status: 400 });
        }
