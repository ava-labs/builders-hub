...
import SelectBlockchain, {
  type BlockchainSelection,
} from "@/components/toolbox/components/SelectBlockchain";
...
export default function TokenBridge() {
  ...
  const [destinationSelection, setDestinationSelection] = useState<BlockchainSelection>({
    blockchainId: "",
    blockchain: null,
  });
    ...
    const fetchSuggestions = async () => {
      ...
      if (erc20TokenHomeAddress) {
        suggestions.push({
          ...
          token: await fetchTokenInfoFromBridgeContract(
            erc20TokenHomeAddress as Address,
            "source",
            false
          ),
        });
      ...
      if (nativeTokenHomeAddress) {
        suggestions.push({
          ...
          token: await fetchTokenInfoFromBridgeContract(
            nativeTokenHomeAddress as Address,
            "source",
            false
          ),
        });
  ...
  const [destinationContractSuggestions, setDestinationContractSuggestions] = useState<
    Suggestion[]
  >([]);
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      if (destToolboxStore?.erc20TokenRemoteAddress) {
        suggestions.push({
          ...
          token: await fetchTokenInfoFromBridgeContract(
            destToolboxStore.erc20TokenRemoteAddress as Address,
            "destination",
            false
          ),
        });
    ...
    destToolboxStore?.erc20TokenRemoteAddress,
    destToolboxStore?.nativeTokenRemoteAddress,
    destL1?.id,
  ]);

  // Fetch token info from bridge contract on current chain
  const fetchTokenInfoFromBridgeContract = useCallback(
    async (address: Address, direction: "source" | "destination", updateState: boolean = true) => {
      if (
        !address ||
        (direction === "source" && !viemChain) ||
        (direction === "destination" && !destL1?.rpcUrl)
      ) {
        return;
      }
      ...
      try {
        ...
        if (direction === "source") {
          ...
          const fetchedTokenAddress = (await publicClient
            .readContract({
              address: address,
        ...
        const [fetchedDecimals, fetchedName, fetchedSymbol, fetchedBalance, fetchedAllowance] =
          await Promise.all([
            ...
            publicClient.readContract({
              ...
              args: [
                walletEVMAddress === "" ? zeroAddress : (walletEVMAddress as Address),
                address as Address,
              ],
            }),
          ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (
        sourceToken.isNative === false &&
        (tokenAllowance === null || tokenAllowance < amountParsed)
      ) {
        setLocalError(
          `Insufficient allowance. Please approve at least ${amount} ${tokenSymbol || "tokens"}.`
        );
      ...
      if (tokenBalance === null || tokenBalance < amountParsed) {
        setLocalError(
          `Insufficient balance. You only have ${formatUnits(tokenBalance ?? 0n, tokenDecimals)} ${tokenSymbol || "tokens"}.`
        );
        setIsProcessingSend(false);
        return;
      }
      ...
      if (receipt.status === "success") {
        const log = receipt.logs.find(
          (l: any) =>
            l.address.toLowerCase() === "0x253b2784c75e510dD0fF1da844684a1aC0aa5fcf".toLowerCase()
        );
  ...
  const isReadyToSend =
    isValidAmount &&
    (hasSufficientAllowance || sourceToken?.isNative === true) &&
      ...
      <div className="flex gap-2 pt-2 mt-4 flex-wrap">
        <Button
          ...
          disabled={
            ...
            isFetchingSourceInfo
          }
          variant={hasSufficientAllowance ? "secondary" : "primary"}
        >
          {hasSufficientAllowance
            ? `Approved (${formatUnits(tokenAllowance ?? 0n, tokenDecimals ?? 18)} ${tokenSymbol})`
            : `1. Approve ${amount || 0} ${tokenSymbol || ""}`}
        </Button>
        <Button
          onClick={handleSend}
          loading={isProcessingSend}
          disabled={
            isProcessingApproval || isProcessingSend || !isReadyToSend || isFetchingSourceInfo
          }
        >
      ...
      {lastSendTxId && lastSendTxDetails && (
        <div className="w-full border rounded-md bg-gray-50 dark:bg-gray-800">
          <div className="flex w-full items-center justify-evenly p-6">
            ...
            {destL1?.logoUrl && (
              <img src={destL1.logoUrl} alt={destL1.name} className="w-8 h-8 rounded-full mx-4" />
            )}
            <div className="flex flex-col items-center gap-1 flex-1 min-w-0">
