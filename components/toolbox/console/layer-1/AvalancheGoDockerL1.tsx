...
import {
  ...
  useGenesisHighlight,
} from "../../components/genesis/GenesisHighlightContext";
...
function AvalanchegoDockerInner() {
  ...
  const [nodeType, setNodeType] = useState<"validator" | "public-rpc" | "validator-rpc">(
    "validator"
  );
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      value={nodeType}
                      onChange={(e) =>
                        setNodeType(e.target.value as "validator" | "public-rpc" | "validator-rpc")
                      }
                    ...
                    {nodeType === "validator-rpc" && (
                      <p className="text-xs text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-1">
                        ...
                        Not recommended for production. Combines validator and RPC for testnet
                        convenience.
                      </p>
                    )}
                  </div>
                  ...
                  {isValidator && (
                    ...
                    >
                      ...
                      />
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        The minimum delay between blocks (in milliseconds). Maximum: 2000ms.
                        Default: 500ms.
                      </p>
                    </div>
                  )}
                  ...
                  <div
                    onMouseEnter={() => setHighlightPath("adminApi")}
                    onMouseLeave={clearHighlight}
                    role="group"
                  >
                    <label className="flex items-center space-x-2">
                    ...
                    <>
                      ...
                      {subnet && subnet.blockchains && subnet.blockchains.length > 1 && (
                        <div>
                          ...
                          >
                            {subnet.blockchains.map(
                              (blockchain: { blockchainId: string; blockchainName: string }) => (
                                <option
                                  key={blockchain.blockchainId}
                                  value={blockchain.blockchainId}
                                >
              ...
              <Accordions type="single" className="mt-4">
                {isCustomVM && (
                  <Accordion title="Custom VM Configuration">
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      This blockchain uses a non-standard Virtual Machine ID. The Docker command
                      includes VM aliases mapping.
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
