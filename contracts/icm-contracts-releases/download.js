    ...
    const request = https.get(url, options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          reject(
            new Error(`Failed to parse JSON: ${e.message}. Data: ${data.substring(0, 100)}...`)
          );
        }
      });
    });
...
async function downloadReleases() {
  try {
    const releases = await getJsonFromURL(
      "https://api.github.com/repos/ava-labs/icm-contracts/releases"
    );
