...
export async function POST(request: Request) {
  try {
    if (!HUBSPOT_API_KEY || !HUBSPOT_PORTAL_ID || !RETRO9000_FORM_GUID) {
      console.error(
        "Missing environment variables: HUBSPOT_API_KEY, HUBSPOT_PORTAL_ID, or RETRO9000_FORM_GUID"
      );
    ...
    const fieldMapping: { [key: string]: string[] } = {
      ...
      avalanche_l1_project_benefited_1_website: [
        "2-44649732/avalanche_l1_project_benefited_1_website",
      ],
    ...
    Object.keys(fieldMapping).forEach((formFieldName) => {
      ...
      if (Array.isArray(value)) {
        ...
        if (value === undefined || value === null || value === "") {
          ...
          const isAmountField = mappedFields.some(
            (field) => field.includes("amount") || field.includes("funding")
          );
