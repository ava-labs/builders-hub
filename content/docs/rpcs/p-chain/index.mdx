...
- `validators` can include different fields based on the subnet type (L1, PoA Subnets, the Primary Network):
  ...
  - `validatorAuthority`, `nextPeriod` and `autoCompoundRewardShares` are returned **only for auto-renewed
    validators**, that is, validators created with `AddAutoRenewedValidatorTx` after the
    [Helicon](/docs/primary-network/helicon-upgrade) upgrade ([ACP-236](/docs/acps/236-auto-renewed-staking)).
    All three are absent for every other validator, so their presence is how you tell an auto-renewed
    validator apart from a fixed-term one. They are flattened into the validator object rather than nested
    under a parent field.
    - `validatorAuthority` is an `OutputOwners` output which includes `locktime`, `threshold` and array of
      `addresses`. It is the owner authorized to change this validator's configuration with a
      `SetAutoRenewedValidatorConfigTx`.
    - `nextPeriod` is the duration in seconds of the next validation cycle.
    - `autoCompoundRewardShares` is the percentage of rewards restaked at the end of each cycle, expressed
      in millionths in the range `[0, 1000000]`. `500000` means half the rewards are restaked and half are
      paid out.
    - `endTime` for an auto-renewed validator is the end of the **current cycle**, not the end of the
      ...
      `nextPeriod`, and `weight` **grows** by whatever share of the rewards was auto-compounded (capped at
      `MaxValidatorStake`). `txID` stays the same across renewals, so do not treat a changed `endTime` as a
      new validator.
...
**Example Response (auto-renewed Primary Network validator, post-Helicon):**
...
Here `nextPeriod` is `172800` seconds (48 hours) and `autoCompoundRewardShares` is `500000`, meaning half of each cycle's rewards are restaked into `weight` and half are paid out to `validationRewardOwner`. `startTime` is Helicon activation on Mainnet and `endTime` is one 48-hour cycle later, so this validator is in its first cycle.
