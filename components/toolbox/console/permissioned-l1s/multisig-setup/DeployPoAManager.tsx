...
const metadata: ConsoleToolMetadata = {
  ...
  description:
    "Deploy and initialize the PoAManager contract to manage Proof of Authority validators",
      ...
      <div className="space-y-4">
        ...
        {Boolean(subnetIdL1) && (
          <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded text-yellow-800 dark:text-yellow-200 text-sm">
            <strong className="font-semibold">Heads up:</strong> Make sure your Validator Manager is
            deployed on the same chain as your{" "}
            <a
            ...
            deployment. You can get your L1 indexed to ensure full compatibility. You can proceed
            with other multisig options, but some console tools may lose compatibility if you do.
          </div>
        )}
        ...
        <Steps>
          <Step>
            ...
            <p className="text-sm text-gray-500">
              Deploy the <code>PoAManager</code> contract with the specified owner and validator
              manager addresses. The contract will be initialized automatically during deployment.
            </p>
            {viemChain && (
              ...
              <div className="space-y-3">
                ...
                {safeSelection.safeAddress && (
                  <div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-2 rounded">
                    <strong>Safe Details:</strong> {safeSelection.threshold}/
                    {safeSelection.owners.length} multisig
                    <br />
                    <strong>Owners:</strong>{" "}
                    {safeSelection.owners.length > 0
                      ? safeSelection.owners.join(", ")
                      : "Loading..."}
                  </div>
                )}
              </div>
            ...
            {poaManagerAddress && (
              <Success label="PoA Manager Deployed & Initialized" value={poaManagerAddress} />
            )}
          </Step>
