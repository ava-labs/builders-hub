    ...
    <div className="mt-6 space-y-4">
      ...
      {latestBlock && (
        <div className="text-sm text-gray-500">
          Showing events from the last {blockLimit.toLocaleString()} blocks (current block:{" "}
      ...
      {error && (
        <div className="p-3 bg-red-50 border border-red-200 rounded text-red-600">{error}</div>
      )}

      {!loading && events.length === 0 && !error && (
        <div className="p-3 bg-gray-50 border border-gray-200 rounded">
          No events found in recent blocks. The contract may have no events or they occurred before
          the query range.
        </div>
      )}
