      ...
      <CardHeader className="pb-3">
        <CardTitle>
          <div className="flex justify-between items-center gap-2">
            <h2 className="text-red-500 text-lg font-bold">{props.track.name}</h2>
            {props.track.icon && props.track.icon.trim() !== "" ? (
              <DynamicIcon name={props.track.icon as any} size={20} className="!text-red-500" />
            ) : (
              <DynamicIcon name="wrench" size={20} className="!text-red-500" />
            )}
          </div>
        </CardTitle>
      </CardHeader>
