...
In order to install a VM, its binary must be installed in the `AvalancheGo` plugin path. See [here](/docs/primary-network/configure/configs-flags#--plugin-dir-string) for more details. Multiple VMs can be installed in this location.
