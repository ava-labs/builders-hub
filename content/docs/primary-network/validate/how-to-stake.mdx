...
- The minimum amount of time one can stake funds for validation is 48 hours (it was 2 weeks before the [Helicon](/docs/primary-network/helicon-upgrade) upgrade, which activates on Mainnet on September 22, 2026)
- The maximum amount of time one can stake funds for validation is 1 year
- The minimum amount of time one can stake funds for delegation is 2 weeks. Helicon does not change this, so it is now longer than the 48-hour validator minimum. A delegation must also fit entirely inside the validator's staking period, which means **a validator staking for less than 2 weeks cannot receive delegations at all**
...
A validator will receive a staking reward if they are online and responsive for more than 90% of their validation period, as measured by a majority of validators, weighted by stake. The threshold was 80% before the [Helicon](/docs/primary-network/helicon-upgrade) upgrade and applies per validation based on its **start time**, so a validation that began before activation is still judged against 80%. **You should aim for your validator be online and responsive 100% of the time.**
