...
* **Native Transfers**
    Move native stablecoins (USDT, USDC, USDe, etc.) across chains without wrapping.
* **Messaging-Agnostic Design**
    Integrate using Allbridge Messaging, Circle’s CCTP, and LayerZero OFT
* **Flexible Fee Options**
    Pay bridge fees in stablecoins or gas tokens for a smoother UX.
* **Optional Gas Top-Up**
    Users can receive AVAX on the destination chain to cover initial transactions.
* **Developer Tools**
    Full SDK and REST API for quick integration into dApps, wallets, and exchanges.
