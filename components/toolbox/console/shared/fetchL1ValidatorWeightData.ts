 ...
 */
export async function fetchL1ValidatorWeightData(
  txId: string,
  isTestnet: boolean
): Promise<L1ValidatorWeightData> {
  ...
  if (!tx?.unsignedTx) {
    throw new Error(
      "Invalid transaction data — is this a SetL1ValidatorWeightTx?"
    );
  }
  ...
  const l1ValidatorWeightPayload =
    extractPayloadFromAddressedCall(addressedCallBytes);
  if (!l1ValidatorWeightPayload) {
    throw new Error(
      "Failed to extract L1ValidatorWeightMessage payload from AddressedCall"
    );
  }

  // Parse the L1ValidatorWeightMessage
  const parsedData = unpackL1ValidatorWeightPayload(
    new Uint8Array(l1ValidatorWeightPayload)
  );
