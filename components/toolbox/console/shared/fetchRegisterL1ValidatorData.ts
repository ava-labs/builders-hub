 ...
 */
export async function fetchRegisterL1ValidatorData(
  txId: string,
  isTestnet: boolean
): Promise<RegisterL1ValidatorData> {
  ...
  if (!tx?.unsignedTx) {
    throw new Error(
      "Invalid transaction data — is this a RegisterL1ValidatorTx?"
    );
  }
  ...
  const registerL1ValidatorPayload =
    extractPayloadFromAddressedCall(addressedCallBytes);
  if (!registerL1ValidatorPayload) {
    throw new Error(
      "Failed to extract RegisterL1ValidatorMessage payload from AddressedCall"
    );
  }

  // Parse the RegisterL1ValidatorMessage
  const parsedData = unpackRegisterL1ValidatorPayload(
    new Uint8Array(registerL1ValidatorPayload)
  );
