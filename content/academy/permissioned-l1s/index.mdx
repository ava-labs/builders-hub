---
title: Welcome to the Course
description: Learn about the L1 Validator Management.
updated: 2025-07-15
authors: [nicolasrnedo]
icon: Smile
---

## Permissioned L1s 

Welcome to the **Permissioned L1s** course! This course is designed to give you a deep
understanding of configuring, launching and maintaining permissioned L1s on Avalanche. By the end of this course, you will have
practical skills in deploying an L1 and managing the validator set in a Proof of Authority (PoA)
network.

## Video

<YouTube id="xgx6VzdR974" params="start=365"/>

### What You'll Learn

This comprehensive course will walk you through:

- **Introduction** - P-Chain & Interchain Messaging for Validator Manager Contract
- **Proof of Authority** - Understanding private vs permissioned vs permissionless networks, what Proof of Authority is and the Validator Manager contract
- **Create an L1** - Create a Subnet, Understanding VMC Deployment Options and creating your L1 (in case you didn't create one in the [Avalanche Fundamentals](/academy/avalanche-fundamentals))
- **Validator Manager Deployment** - Deploying and configuring Validator Manager Contracts (VMC)
- **Validator Management** - Adding, removing, and changing validator weights with P-Chain integration
- **Coming Soon...** Multi-Sig Setup for PoA - Implementing secure multi-signature governance with Safe/Ash wallets
- **Coming Soon...** Private L1s - Configuring validator-only access and RPC node restrictions

### Let's Get Started!

Each section builds upon previous knowledge, so we recommend completing the course in order.

Happy learning and building!