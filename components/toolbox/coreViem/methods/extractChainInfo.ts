...
//TODO: rename
export async function extractChainInfo(
  client: AvalancheWalletClient,
  { txId }: ExtractChainInfoParams
): Promise<ExtractChainInfoResponse> {
  ...
  if (!data?.tx?.unsignedTx) {
    throw new Error(
      "Received unexpected response from node: " + JSON.stringify(data).slice(0, 150)
    );
  }
