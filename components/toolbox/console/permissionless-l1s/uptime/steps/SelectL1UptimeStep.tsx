...
export default function SelectL1UptimeStep() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
      <div className="space-y-4">
        ...
        <p className="text-sm text-zinc-500 mb-4">Choose the L1 subnet to view validator uptimes and submit proofs.</p>
        <SelectSubnetId
          value={store.subnetIdL1}
          onChange={(value) => store.setSubnetIdL1(value)}
          error={vmcCtx.error}
        ...
        {vmcCtx.ownerType &&
          ...
          !vmcCtx.staking?.isLoading && (
            <Alert variant="error">
              This L1 is not using a Staking Manager. Uptime proofs are only relevant for permissionless L1s.
            </Alert>
          )}
      </div>
      {store.subnetIdL1 && (
