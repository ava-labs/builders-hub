...
function GenesisBuilderInner({
  ...
  useEffect(() => {
    if (walletEVMAddress && tokenAllocations.length === 1 && tokenAllocations[0].address === PLACEHOLDER_ADDRESS) {
  ...
  const [contractDeployerAllowListConfig, setContractDeployerAllowListConfig] = useState<AllowlistPrecompileConfig>(
    generateEmptyAllowlistPrecompileConfig(),
  );
  const [contractNativeMinterConfig, setContractNativeMinterConfig] = useState<AllowlistPrecompileConfig>(
    generateEmptyAllowlistPrecompileConfig(),
  );
  ...
  useEffect(() => {
    ...
    if (!evmChainId || evmChainId < 1) errors.chainId = 'Chain ID must be a positive integer';
      ...
      [1, 43113, 43114, 53935, 73772, 78430, 78431, 78432, 78433, 78434, 78435, 78436, 78437, 78438, 78439].includes(
        evmChainId,
      )
    ) {
    ...
    else if (gasLimit > HIGH_GAS_LIMIT_THRESHOLD)
      warnings.gasLimit = 'High gas limits require more validator resources';
    ...
    if (!isValidAllowlistPrecompileConfig(feeManagerConfig))
      errors.feeManager = 'Fee Manager: Configuration is invalid or requires at least one valid address.';
    if (!isValidAllowlistPrecompileConfig(rewardManagerConfig))
      errors.rewardManager = 'Reward Manager: Configuration is invalid or requires at least one valid address.';
    ...
    if (feeConfig.blockGasCostStep > 5000000)
      warnings.blockGasCostStep = 'Block gas cost step above 5M may cause fees to change too rapidly';
            ...
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs">
                <div className="space-y-2">
                  <p className="text-xs">
                    A unique identifier for your blockchain network. Choose an ID that doesn't conflict with existing
                    chains.
                  </p>
                  <a
