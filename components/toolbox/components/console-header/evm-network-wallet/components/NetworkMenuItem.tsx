    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {!isEditMode && isActive && !isTokenSelected && !hasTokens && <Check className="w-4 h-4 text-green-600" />}
        </div>
      </DropdownMenuItem>
      ...
      {!isEditMode && hasTokens && isExpanded && (
        <>
          {/* Native token option */}
          <DropdownMenuItem onSelect={handleNativeSelect} className="pl-12 pr-3 py-2 cursor-pointer">
            <div className="flex items-center gap-2 flex-1">
              ...
              {isActive && selectedToken === null && <Check className="w-4 h-4 text-green-600" />}
            </div>
          </DropdownMenuItem>
          ...
          {network.wellKnownERC20s?.map((token) => (
            ...
            >
              <div className="flex items-center gap-2 flex-1">
                ...
                {isActive && selectedToken === token.address && <Check className="w-4 h-4 text-green-600" />}
              </div>
            </DropdownMenuItem>
          ))}
        </>
