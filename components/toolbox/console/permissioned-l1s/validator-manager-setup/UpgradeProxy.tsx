...
function UpgradeProxy({ onSuccess }: BaseConsoleToolProps) {
  ...
  async function checkCurrentImplementation() {
    try {
      ...
      setCurrentImplementation(null);
      const errorMessage =
        error instanceof Error ? error.message : "Failed to read current implementation";
  ...
  const canUpgrade =
    !!proxyAddress && !!proxyAdminAddress && !!desiredImplementation && isUpgradeNeeded;
