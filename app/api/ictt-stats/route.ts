...
function getTokenInfo(address: string): TokenInfo {
  ...
  const token =
    (icttTokens as Record<string, TokenInfo>)[normalizedAddress] || (icttTokens as Record<string, TokenInfo>)[address];
...
function getChainInfoByBlockchainId(blockchainId: string) {
  ...
  const chain = (l1ChainsData as ChainData[]).find((c) => c.blockchainId === blockchainId);

  return chain;
}

// Fetch prices from CoinGecko
async function fetchTokenPrices(coingeckoIds: string[]): Promise<Record<string, number>> {
  ...
  try {
    ...
    if (!response.ok) {
      // CoinGecko API error; return empty prices
      return {};
    }
    ...
    // Token price fetch failed; return empty
    return {};
  }
}
...
export const GET = withApi(async (req) => {
  ...
  // Check cache
  if (!clearCache && cachedData && Date.now() - cachedData.timestamp < CACHE_DURATION) {
    const paginatedData = {
      ...cachedData.data,
  ...
  const icttResponse = await fetch(`https://idx6.solokhin.com/api/global/ictt/transfers?startTs=0&endTs=${endTs}`);
  ...
  transfers.forEach((transfer) => {
    ...
    if (tokenInfo.coingeckoId) {
      ...
      tokenAddressToCoingeckoId[transfer.coinAddress.toLowerCase()] = tokenInfo.coingeckoId;
    }
  });
  ...
  const totalTransfers = transfers.reduce((sum, t) => sum + t.transferCount, 0);

  const totalVolumeUsd = enrichedTransfers.reduce((sum, t) => sum + t.volumeUsd, 0);
  ...
  const topToken = Object.values(tokenCounts).sort((a, b) => b.count - a.count)[0];

  const topTokenPercentage = totalTransfers > 0 ? ((topToken.count / totalTransfers) * 100).toFixed(1) : '0';

  // Get top routes
  const routes: Record<string, { name: string; total: number; direction: string }> = {};
  enrichedTransfers.forEach((t) => {
    ...
    if (!routes[routeKey]) {
      ...
      routes[routeKey] = {
        name: t.direction === 'out' ? `${homeName} → ${remoteName}` : `${remoteName} → ${homeName}`,
        total: 0,
  ...
  const sortedTransfers = enrichedTransfers.sort((a, b) => b.transferCount - a.transferCount);
  ...
  const { allTransfers: _allTransfers, ...baseData } = fullResponseData;
  const responseData = {
    ...baseData,
