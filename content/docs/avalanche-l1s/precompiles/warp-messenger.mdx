...
edit_url: https://github.com/ava-labs/avalanchego/edit/master/graft/subnet-evm/precompile/contracts/warp/README.md
---
...
Like other precompiles, the Warp Messenger must be activated in the genesis file or through a network upgrade. It is commonly enabled from genesis on L1s that need cross-chain messaging:
...
- `quorumNumerator`: The percentage of stake weight required to verify a message (default: 67, meaning 67%). Values below 33 are rejected
- `requirePrimaryNetworkSigners`: Whether messages from the C-Chain and X-Chain must be verified against the Primary Network validator set. Required for L1s that need to receive messages from those chains (default: `false`)
...
interface IWarpMessenger {
  ...
  // getVerifiedWarpBlockHash returns the verified Warp block hash if it exists, otherwise returns (WarpBlockHash, false)
...
The Warp Precompile is broken down into four functions defined in the Solidity interface file [here](https://github.com/ava-labs/avalanchego/blob/master/graft/subnet-evm/precompile/contracts/warp/warpbindings/IWarpMessenger.sol).
...
`getVerifiedWarpMessage` is used to read the contents of the delivered Avalanche Warp Message into the expected format.
...
This pre-verification is performed using the ProposerVM Block header during [block verification](https://github.com/ava-labs/avalanchego/blob/master/graft/subnet-evm/plugin/evm/block.go) and [block building](https://github.com/ava-labs/avalanchego/blob/master/graft/subnet-evm/miner/worker.go).

### getVerifiedWarpBlockHash

`getVerifiedWarpBlockHash` reads a signed block hash from a source chain, delivered through the same predicate mechanism as `getVerifiedWarpMessage`.

It returns the `WarpBlockHash` if present and a boolean indicating if one is present.

This allows a contract to verify that a specific block was accepted on the source chain, without requiring a message to have been explicitly sent from it.
...
Therefore, we use the [Predicate Utils](https://github.com/ava-labs/avalanchego/blob/master/graft/subnet-evm/predicate/Predicate.md) package to encode the actual byte slice of size N into the access list.
