...
function AddCollateral() {
  ...
  const fetchTokenHomeInfo = useCallback(async () => {
    ...
    try {
      ...
      const blockchainIdBase58 = utils.base58check.encode(
        Buffer.from(fetchedTokenHomeBlockchainID.slice(2), "hex")
      );
  ...
  useEffect(() => {
    if (
      collateralInfo?.needed &&
      collateralInfo.needed > 0n &&
      tokenDecimals !== null &&
      !isAutoFilled
    ) {
  ...
  const handleAddCollateral = async () => {
    ...
    try {
      ...
      if (tokenType === "native") {
        simulateParams.args = [
          remoteBlockchainIDHex as `0x${string}`,
          remoteContractAddress as Address,
        ];
    ...
    <div className="mt-8 space-y-4">
      ...
      <div className="flex gap-2 pt-2 border-t mt-4 flex-wrap">
        ...
        <Button
          ...
          loading={
            isProcessing && !lastAddCollateralTxId && (tokenType === "native" || !!lastApprovalTxId)
          }
          disabled={
            isProcessing ||
            !isValidAmount ||
