...
export default function RequireWalletConnection({
  ...
  const renderContent = () => {
    switch (connectionStatus) {
      case "no_wallet_installed":
        return (
          <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
            ...
            <p className="mb-4">You need an Core Wallet to launch your L1:</p>
            <a
      ...
      case "correct_chain_connected":
        return <>{children ? children : `Successfully connected to ${chain.name}!`}</>;
      default:
  ...
  if (!chain.rpcUrls?.default?.http[0]) {
    return (
      <p>
        No RPC URL found for {chain?.name || "unknown chain"}. This is likely a bug, please contact
        support.
      </p>
    );
  }
