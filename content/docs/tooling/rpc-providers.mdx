...
[here](/docs/tooling/avalanche-postman/making-api-calls). Namely, use the following end points for
...
[X-Chain](/docs/apis/x-chain/api), [P-Chain](/docs/apis/p-chain/api),
[C-Chain](/docs/apis/c-chain/api), and full archival for the Primary Network.
However, it doesn't support [Index APIs](/docs/apis/index-api), which includes
the X-Chain API's `getAddressTxs` method.

For a full list of available APIs see [here](/docs/apis/p-chain/api).
...
The maximum number of blocks to serve per `getLogs` request is 2048, which is set by [`api-max-blocks-per-request`](/docs/primary-network/chain-configs/c-chain#api-max-blocks-per-request).
