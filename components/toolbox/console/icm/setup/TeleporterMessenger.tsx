...
const metadata: ConsoleToolMetadata = {
  ...
  description:
    "Deploy the ICM messenger contract to your L1 to enable cross-L1 messaging and applications like ICTT",
...
const TopUpComponent = ({
  ...
  return (
    <Step>
      ...
      <p>
        The deployer address needs at least {formatEther(MINIMUM_BALANCE)} native coins to send the
        transaction.
      </p>
      <div className="flex items-center gap-2">
        ...
        />
        <Button variant="primary" onClick={handleTopUp} loading={isSending} disabled={isSending}>
          Send Funds
        </Button>
      </div>
    </Step>
...
function TeleporterMessenger({ onSuccess }: BaseConsoleToolProps) {
  ...
  return (
    <>
      <div>
        <p className="mt-2">
          This tool deploys the TeleporterMessenger contract, which is the core contract that
          handles cross-subnet message sending and receiving. Please read more{" "}
          <a
      ...
      <p className="text-sm text-gray-500 mb-4">
        Contract source:{" "}
        <a href={TELEPORTER_MESSENGER_SOURCE_URL} target="_blank" rel="noreferrer">
      ...
      <Steps>
        <Step>
          ...
          <p className="text-sm text-gray-500">Enter the parameters for your new chain.</p>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="font-semibold">Deployer Address:</p>
                <code className="block py-2 rounded text-sm break-all">{deployerAddress}</code>
            ...
            {!isDeployed && (
              <div>
                ...
                {isCheckingBalance ? (
                  ...
                  <p>
                    {formatEther(deployerBalance)} coins {hasEnoughBalance ? "✅" : "❌"}
                  </p>
                )}
                <div className="pb-2 text-xs">
            ...
            {!hasEnoughBalance && !isDeployed && (
              <TopUpComponent deployerAddress={deployerAddress} onTopUp={checkDeployerBalance} />
            )}
          </div>
        </Step>
        <Step>
      ...
      {isDeployed && (
        <Success label="TeleporterMessenger Address" value={expectedContractAddress} />
      )}
    </>
  );
}
