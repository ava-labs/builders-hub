...
function RevertPoAManager({ onSuccess }: BaseConsoleToolProps) {
  ...
  const handleTransferSuccess = (message: string) => {
  ...
  const canTransfer =
    ...
    !!poaManagerAddress &&
    !!newOwnerAddress &&
    poaManagerAddress?.toLowerCase() !== newOwnerAddress?.toLowerCase() &&
      ...
      <Steps>
        <Step>
          ...
          <p className="text-sm text-gray-500">
            Select the L1 whose ValidatorManager you want to revert from PoAManager control. This
            will transfer ownership to a new address via the PoAManager contract.
          </p>
                ...
                <>
                  ...
                  {ownerType === "EOA" && (
                    <p className="text-xs text-yellow-600 mt-1">
                      ⚠️ The ValidatorManager is owned by an EOA, not a PoAManager. This tool is for
                      reverting from PoAManager control.
                    </p>
                  )}
                  {ownerType === "StakingManager" && (
                    <p className="text-xs text-yellow-600 mt-1">
                      ⚠️ The ValidatorManager is owned by a StakingManager contract. This tool is
                      specifically for PoAManager contracts.
                    </p>
                  )}
                </>
              )}
        ...
        {ownerType === "PoAManager" && (
          <Step>
            ...
            <p className="text-sm text-gray-500">
              ...
              <code>transferValidatorManagerOwnership(newOwner)</code> on the PoAManager to revert
              control.
            </p>
