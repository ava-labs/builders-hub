...
export function LockedContent({
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      <div className={cn('transition-all duration-300', !isUnlocked && 'blur-[4px] pointer-events-none select-none')}>
        {children}
      </div>
      ...
      {!isUnlocked && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/30 dark:bg-black/30 backdrop-blur-[1px]">
          <div className="text-center max-w-md flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-zinc-900 dark:text-white drop-shadow-sm w-75">{lockedMessage}</p>
          </div>
        </div>
      )}
    </div>
