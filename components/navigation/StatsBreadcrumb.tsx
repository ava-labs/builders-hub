...
function ChainLogo({ src, name, size = "sm" }: { src?: string; name: string; size?: "sm" | "md" }) {
  const imgSizeClasses = size === "sm" ? "w-4 h-4" : "w-3 h-3 sm:w-3.5 sm:h-3.5";
  ...
  return <div className={containerClasses}>{getChainInitials(name)}</div>;
}
...
export function StatsBreadcrumb({
  ...
  return (
    <>
      ...
      <nav
        ref={navRef}
        className={`stats-breadcrumb flex items-center gap-1.5 text-xs sm:text-sm mb-3 sm:mb-4 pb-1 ${className}`}
      >
          ...
          <>
            ...
            {availableChains.length > 0 ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  align="start"
                  className="max-h-[500px] w-64 flex flex-col p-0"
                  onCloseAutoFocus={() => setChainSearchTerm("")}
                >
          ...
          <>
            ...
            {availableChains.length > 0 ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    className={`inline-flex items-center gap-1 sm:gap-1.5 px-3 py-1.5 rounded-md ${breadcrumbItems.length > 0 ? "text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400"} hover:bg-zinc-200 dark:hover:bg-zinc-700 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors cursor-pointer whitespace-nowrap flex-shrink-0`}
                  >
                    {chainSlug === "all-chains" ? (
                ...
                >
                  ...
                  <div className="overflow-y-auto flex-1 py-1">
                    ...
                    {(() => {
                      const filteredCustomChains = customChains.filter(
                        (chain) =>
                          !chainSearchTerm ||
                          chain.chainName.toLowerCase().includes(chainSearchTerm.toLowerCase())
                      );
