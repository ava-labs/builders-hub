...
export async function transferCToP(
  amount: string,
  privateKeyHex: string,
  setNewPChainBalance: (balance: string) => void
) {
  ...
  if (!imported) {
    throw new Error(
      "Export transaction may still be processing - no UTXOs found to import after 3 attempts"
    );
  }
