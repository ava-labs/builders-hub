...
const bytesToHexPrefixed = (bytes: Uint8Array): `0x${string}` =>
...
function unpackSubnetToL1ConversionMessage(input: Uint8Array): Uint8Array {
  ...
  if (input.length !== EXPECTED_LENGTH) {
    throw new Error(
      `Invalid message length. Expected ${EXPECTED_LENGTH} bytes, got ${input.length}`
    );
  }
  ...
  if (typeID !== SUBNET_TO_L1_CONVERSION_MESSAGE_TYPE_ID) {
    throw new Error(
      `Invalid message type: ${typeID}, expected ${SUBNET_TO_L1_CONVERSION_MESSAGE_TYPE_ID}`
    );
  }
...
function packConversionData(conversionData: SolidityConversionData): Uint8Array {
  ...
  if (conversionData.validatorManagerBlockchainID.length !== 32) {
    throw new Error("validatorManagerBlockchainID must be 32 bytes");
  }
  ...
  if (managerAddressBytes.length !== 20) {
    throw new Error("validatorManagerAddress must be 20 bytes hex string");
  }
  ...
  for (const validator of sortedValidators) {
    if (validator.blsPublicKey.length !== 48) {
      throw new Error("Validator blsPublicKey must be 48 bytes");
    }
...
export function marshalSubnetToL1ConversionData(args: PackL1ConversionMessageArgs): Uint8Array {
  ...
  for (const validator of sortedValidators) {
    ...
    if (blsPublicKeyBytes.length !== 48) {
      console.warn(
        `Expected BLS public key (nodePOP.publicKey) to be 48 bytes, got ${blsPublicKeyBytes.length}`
      );
...
export function unpackRegisterL1ValidatorPayload(input: Uint8Array): SolidityValidationPeriod {
  ...
  if (index !== input.length) {
    ...
    const expectedLength =
      ...
      4 +
      32 +
      (4 + validation.nodeID.length) +
      48 +
      8 +
      (4 + 4 + remainingBalanceOwnerAddressesLength * 20) +
      (4 + 4 + disableOwnerAddressesLength * 20) +
      8;
    throw new Error(
      `Invalid message length: parsed ${index} bytes, expected ${expectedLength}, total length ${input.length}`
    );
  }
...
function packL1ValidatorRegistrationPayload(
...
function unpackL1ValidatorRegistrationPayload(payload: Uint8Array): L1ValidatorRegistration {
  ...
  if (payload.length !== EXPECTED_LENGTH) {
    throw new Error(
      `Invalid L1ValidatorRegistration payload length. Expected ${EXPECTED_LENGTH} bytes, got ${payload.length}`
    );
  }
...
export function unpackL1ValidatorWeightPayload(payload: Uint8Array): L1ValidatorWeight {
  ...
  if (payload.length !== EXPECTED_LENGTH) {
    throw new Error(
      `Invalid L1ValidatorWeight payload length. Expected ${EXPECTED_LENGTH} bytes, got ${payload.length}`
    );
  }

  let offset = 0;
  const codecID = parseUint16(payload, offset);
  offset += 2;
  if (codecID !== CODEC_ID) {
    throw new Error(`Invalid codec ID: ${codecID}`);
  }

  const typeID = parseUint32(payload, offset);
  offset += 4;
  if (typeID !== L1_VALIDATOR_WEIGHT_MESSAGE_TYPE_ID) {
    throw new Error(`Invalid message type: ${typeID}`);
  }

  const validationID = parseBytes(payload, offset, 32);
  offset += 32;
  const nonce = parseUint64(payload, offset);
  offset += 8;
  const weight = parseUint64(payload, offset);
  offset += 8;
...
function unpackValidationUptimePayload(payload: Uint8Array): ValidationUptime {
  ...
  if (payload.length !== EXPECTED_LENGTH) {
    throw new Error(
      `Invalid ValidationUptime payload length. Expected ${EXPECTED_LENGTH} bytes, got ${payload.length}`
    );
  }
  ...
  if (typeID !== VALIDATION_UPTIME_MESSAGE_TYPE_ID) {
    throw new Error(
      `Invalid message type: ${typeID}, expected ${VALIDATION_UPTIME_MESSAGE_TYPE_ID}`
    );
  ...
  const validationID = parseBytes(payload, offset, 32);
  offset += 32;
  const uptime = parseUint64(payload, offset);
  offset += 8;
...
export function newUnsignedMessage(
  networkID: number,
  sourceChainID: string,
  message: Uint8Array
): Uint8Array {
...
export const compareNodeIDs = (a: string, b: string) => {
  ...
  try {
    ...
    aNodeID = a.startsWith("NodeID-")
      ? utils.base58check.decode(a.split("-")[1])
      : utils.hexToBuffer(a);
    bNodeID = b.startsWith("NodeID-")
      ? utils.base58check.decode(b.split("-")[1])
      : utils.hexToBuffer(b);
  } catch (error: any) {
    // Fallback to string comparison if parsing fails (e.g., invalid format)
    console.warn(
      `Failed to parse NodeIDs for comparison ('${a}', '${b}'), falling back to string compare: ${error.message}`
    );
...
export function packL1ConversionMessage(
  args: PackL1ConversionMessageArgs,
  networkID: number,
  sourceChainID: string
  ...
  const unsignedMessage = newUnsignedMessage(
    networkID,
    sourceChainID,
    subnetConversionAddressedCall
  );
