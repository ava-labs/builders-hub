...
description: Understand the AWM Relayer configuration in detail.
updated: 2025-05-13
...
A relayer is configured to relay messages between certain source and destination L1s. The configuration consists of three parts:
...
Let's go through an example JSON understanding the different values:
...
Next is the configuration for the source blockchain. This is the configuration of the blockchain where messages will be initiated and picked up. The relayer will aggregate the signatures of the validators of that L1.
...
Next is the configuration for the destination blockchain. This is the configuration of the blockchain where messages will be sent to. The relayer will aggregate the signatures of the validators of that L1.
```json
{
  "destination-blockchains": [
    {
...
For each destination L1, the relayer has the following config parameters:
...
Some configuration fields have been omitted for the purpose of this course. If you are interested to read the extensive list of relayer configurations, you can visit the awm-relayer GitHub repository [here](https://github.com/ava-labs/awm-relayer/tree/main?tab=readme-ov-file#configuration).
...
The hosted relayer and ICM Relayer tool in Console automatically handle this two-way configuration for you.
