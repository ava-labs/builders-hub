...
export async function POST(request: Request) {
  try {
    if (!HUBSPOT_API_KEY || !HUBSPOT_PORTAL_ID || !HUBSPOT_HACKATHON_FORM_GUID) {
      console.error(
        "Missing environment variables: HUBSPOT_API_KEY, HUBSPOT_PORTAL_ID, or HUBSPOT_HACKATHON_FORM_GUID"
      );
    ...
    processedFormData["avalanche_ecosystem_member"] = formData.avalanche_ecosystem_member
      ? "Yes"
      : "No";

    // Map boolean fields
    processedFormData["marketing_consent"] =
      formData.newsletter_subscription === true ? "Yes" : "No";
