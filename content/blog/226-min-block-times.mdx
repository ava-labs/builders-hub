...
topics: [ACP, Block Time, C-Chain, Consensus, Performance]
comments: true
---
...
ACP-226 proposes swapping the old per-block gas cost for a simple, enforced minimum wait between blocks. New header fields — `timeMilliseconds`
and `minimumBlockDelay` — make sub-second blocks possible and keep spacing consistent. The delay naturally gravitates toward
the stake-weighted median preference of all validators (same control style as ACP-176), so cadence aligns with what validators actually want.

## Why Change the Current Mechanism?

The current gas check mechanism is overly simple and does not reliably prevent exceeding the target rate when priority fees already cover the
...
mechanism still sets the target gas rate, ACP-226 reinforces network safety and prepares the network for higher targets
and smoother UX.

## Looking Ahead

As gas targets climb, expect smaller, more frequent blocks and a snappier user experience. Parallel block checks (like SAE) and ongoing
...
- Community meeting recording: https://www.youtube.com/watch?v=v792NyX58sI
