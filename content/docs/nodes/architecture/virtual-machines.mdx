...
| Transaction | Purpose |
|-------------|---------|
| `AddValidatorTx` | Add a Primary Network validator |
| `AddDelegatorTx` | Delegate stake to a validator |
| `CreateSubnetTx` | Create a new subnet |
| `CreateChainTx` | Launch a blockchain on a subnet |
| `TransformSubnetTx` | Convert subnet to Avalanche L1 |
...
<Callout>
Coreth is developed in [`ava-labs/coreth`](https://github.com/ava-labs/coreth) and vendored into AvalancheGo under `graft/coreth`, so it ships with the node binary by default.
</Callout>
...
- Atomic transactions with other Avalanche chains
...
Snowman++ is enabled on Snowman chains (P-Chain, C-Chain) to pace block production without sacrificing liveness.
