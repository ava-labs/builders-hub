  ...
  return (
    <>
      <div className="space-y-4">
        <div className="">
          This will deploy the <code>ICMDemo</code> contract to your connected network (Chain ID:{" "}
          <code>{selectedL1?.evmChainId}</code>). This contract can receive messages from the
          C-Chain using Avalanche's Inter-Chain Messaging (ICM) protocol. Once deployed, you can use
          the pre-deployed sender contract on the C-Chain at address{" "}
          <a
          ...
          to send messages to this receiver.
        </div>
        <div className="">
          Read more about the{" "}
          <a
            href="https://build.avax.network/academy/interchain-messaging/04-icm-basics/04-create-sender-contract"
            target="_blank"
            className="text-blue-500 hover:underline"
          >
            Sender Contract
          ...
          in the Avalanche documentation.
        </div>
        {!isTeleporterDeployed && (
          <div className="text-red-500">
            TeleporterMessenger contract is not deployed on this network. Please{" "}
            <a href="#teleporterMessenger" className="text-blue-500 hover:underline">
              deploy the TeleporterMessenger contract first
            </a>
            .
          </div>
        )}
        {isTeleporterDeployed && (
          <div>
            ✅ TeleporterMessenger contract is detected at address{" "}
            <code>{TeleporterMessengerAddress.content}</code>.
          </div>
        )}
        <Button
        ...
        <Success label="ICMDemo Address" value={icmReceiverAddress} />
      </div>
    </>
  );
}
