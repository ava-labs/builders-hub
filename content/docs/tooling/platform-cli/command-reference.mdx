...
| `--private-key` | `-k` | Private key (`PrivateKey-...` or `0x...`) | |
...
| Flag | Description |
|------|-------------|
| `--name` | Key name (required, 1-64 alphanumeric characters) |
| `--encrypt` | Encrypt with password (AES-256-GCM + Argon2id) |

### keys import

```bash
platform keys import --name <name> [--private-key <key>] [--encrypt]
```
...
```bash
platform keys export --name <name> [--format <format>]
```
