...
function QueryL1ValidatorSetInner({ onSuccess }: BaseConsoleToolProps) {
  ...
  const handleSearch = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const filtered = validatorsWithWeight.filter((v) => v.nodeId.toLowerCase().includes(term));
    ...
    <>
      <SDKCodeViewer sources={sdkSources} height="auto">
        <div className="space-y-4">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">Subnet ID is required to query L1 validators</p>

          <BlockchainDetailsDisplay subnet={subnet} isLoading={isLoadingSubnet} error={subnetError} />
