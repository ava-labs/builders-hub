...
const metadata: ConsoleToolMetadata = {
  ...
  description:
    "Send a test message between L1s using Avalanche's Inter-Chain Messaging (ICM) protocol",
...
function SendICMMessage({ onSuccess }: BaseConsoleToolProps) {
  ...
  const isQueryButtonDisabled =
    isQuerying || !targetToolboxStore.icmReceiverAddress || !targetL1?.rpcUrl;
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="space-y-1">
          <Success label="Transaction ID (on Source Chain)" value={lastTxId ?? ""} />
        </div>
