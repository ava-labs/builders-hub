...
// ---------------------------------------------------------------------------
// GET /api/build-games/status  — optional auth (unauthenticated = not participant)
// ---------------------------------------------------------------------------

export const GET = withApi(async (_req: NextRequest, { session }) => {
  if (!session?.user?.email) {
    return successResponse({ isParticipant: false });
  }
  ...
  const [application, registration, projects] = await Promise.all([
    ...
    prisma.registerForm.findFirst({
      where: { hackathon_id: BG_HACKATHON_ID, email },
      select: { id: true, name: true, created_at: true },
    }),
    prisma.project.findMany({
      where: {
        hackaton_id: BG_HACKATHON_ID,
        members: { some: { email, status: { not: 'Removed' } } },
      },
      select: {
        id: true,
        project_name: true,
        created_at: true,
        members: {
          where: { email, status: { not: 'Removed' } },
          select: { status: true },
        },
      },
    }),
  ...
  const createdAt =
    (application?.created_at ?? registration?.created_at ?? firstProject?.createdAt)?.toISOString() ??
