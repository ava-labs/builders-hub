...
export const GET = withApi(async (req) => {
  ...
  // ──────────────────────────────────────────────────────────────
  const [watermarkResult, currStatsResult, prevStatsResult, dailyStatsResult, totalChainStats, nativeDeployResult] =
    await Promise.all([
  ...
  for (const [protocol] of protocolAddresses) {
    currentProtocolStats.set(protocol, {
      txCount: 0,
      gasUsed: 0,
      avaxBurned: 0,
      gasCostUsd: 0,
    ...
    prevProtocolStats.set(protocol, {
      txCount: 0,
      gasUsed: 0,
      avaxBurned: 0,
      gasCostUsd: 0,
  ...
  const dailyCatAgg = new Map<
    string,
    { categories: Map<string, number>; totalGas: number; weightedGasPriceSum: number }
  >();
