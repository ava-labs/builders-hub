...
export function useAccountRequirements(
  configKey: AccountRequirementsConfigKey | AccountRequirementsConfigKey[]
): {
  ...
  const collectAllRequirements = useCallback(
    (keys: AccountRequirementsConfigKey[]): AccountRequirementsConfigKey[] => {
  ...
  const requirements: Requirement[] = useMemo(() => {
    const requestedKeys: AccountRequirementsConfigKey[] = Array.isArray(configKey)
      ? configKey
      : [configKey];
    ...
    return allKeys.map((key: AccountRequirementsConfigKey): Requirement => {
      ...
      let met = false;
      let waiting = false;
      const resolvedAction: RequirementAction | null = requirement.action;
