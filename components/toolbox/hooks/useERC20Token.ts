...
import { parseEther, formatEther, PublicClient, Abi } from "viem";
 ...
 */
export function useERC20Token(tokenAddress: string | null, abi: Abi): ERC20TokenHook {
  ...
  const allowance = async (owner: string, spender: string): Promise<string> => {
    ...
    const allowanceAmount = await readContract(avalancheWalletClient as PublicClient, {
  ...
  const balanceOf = async (account: string): Promise<string> => {
    ...
    const balance = await readContract(avalancheWalletClient as PublicClient, {
      address: tokenAddress as `0x${string}`,
      abi: abi,
  ...
  const totalSupply = async (): Promise<string> => {
    ...
    const supply = await readContract(avalancheWalletClient as PublicClient, {
      address: tokenAddress as `0x${string}`,
      abi: abi,
  ...
  const name = async (): Promise<string> => {
    ...
    return (await readContract(avalancheWalletClient as PublicClient, {
      address: tokenAddress as `0x${string}`,
      abi: abi,
      functionName: "name",
      args: [],
  ...
  const approve = (spender: string, amount: string): Promise<string> => {
  ...
  const transfer = (to: string, amount: string): Promise<string> => {
    if (!coreWalletClient || !tokenAddress || !walletEVMAddress || !viemChain) {
      throw new Error("Wallet not connected or contract not ready");
    }
  ...
  const transferFrom = (from: string, to: string, amount: string): Promise<string> => {
    if (!coreWalletClient || !tokenAddress || !walletEVMAddress || !viemChain) {
      throw new Error("Wallet not connected or contract not ready");
    }
