...
export default function TokenAllocationList({ allocations, onAllocationsChange, compact }: TokenAllocationListProps) {
  ...
  const totalSupply = useMemo(
    () => allocations.reduce((sum, a) => sum + (isNaN(a.amount) ? 0 : a.amount), 0),
    [allocations],
  );
          ...
          <div className={`${compact ? 'p-3' : 'p-4'} bg-zinc-50/80 dark:bg-zinc-900/40`}>
            <div className={`flex items-center gap-3`}>
              <div className="flex-1 min-w-0 relative">
                ...
                />
                <Plus
                  className={`absolute left-2 top-1/2 -translate-y-1/2 ${compact ? 'h-3.5 w-3.5' : 'h-4 w-4'} text-zinc-400`}
                />
              </div>
              <RawInput
                type="text"
