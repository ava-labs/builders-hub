...
import {
  ...
  getEventByTopic,
  decodeFunctionInput,
} from "@/abi/event-signatures.generated";
...
function extractERC20Transfers(
...
async function fetchTokenMetadata(
  chainId: string,
  tokenAddress: string
): Promise<{ logoUri?: string; symbol?: string }> {
  try {
...
function extractCrossChainTransfers(
  logs: Array<{ topics: string[]; data: string; address: string }>
): CrossChainTransfer[] {
  ...
  for (const log of logs) {
    ...
    try {
      ...
      if (inputParam?.components) {
        const destChainComp = inputParam.components.find(
          (c) => c.name === "destinationBlockchainID"
        );
        const destAddrComp = inputParam.components.find(
          (c) => c.name === "destinationTokenTransferrerAddress"
        );
        const recipientComp =
          inputParam.components.find((c) => c.name === "recipient") ||
...
export default function TransactionDetailPage({
  ...
  const [erc20Transfers, setErc20Transfers] = useState<
    Array<ERC20Transfer & { symbol: string; decimals: number; logoUri?: string }>
  >([]);
  ...
  useEffect(() => {
    ...
    const extractAndFetch = async () => {
      ...
      const uniqueTokenAddresses = Array.from(
        new Set(transfers.map((t) => t.tokenAddress.toLowerCase()))
      );
      ...
      const transfersWithInfo = transfers.map((transfer) => {
        const tokenInfo = tokenInfoMap.get(transfer.tokenAddress.toLowerCase()) || {
          symbol: "UNKNOWN",
          decimals: 18,
        };
        return {
                  ...
                  value={
                    <div className="space-y-3">
                      {erc20Transfers.map((transfer, idx) => (
                        <div
                          key={idx}
                          className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-sm p-2 rounded-lg bg-zinc-50 dark:bg-zinc-800/50"
                        >
                    ...
                    value={
                      <div className="space-y-3">
                        {crossChainTransfers.map((transfer, idx) => {
                          const destChain = getChainFromBlockchainId(
                            transfer.destinationBlockchainID
                          );
                              ...
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {destChain ? (
                                  <Link
                                    ...
                                    className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium hover:underline cursor-pointer"
                                    style={{
                                      backgroundColor: `${destChain.color}20`,
              ...
              {showMore && (
                <>
                  ...
                  <DetailRow
                    ...
                    value={
                      <span className="text-sm text-zinc-900 dark:text-white">
                        {tx?.gasLimit ? parseInt(tx.gasLimit).toLocaleString() : "-"} |{" "}
                        {tx?.gasUsed ? parseInt(tx.gasUsed).toLocaleString() : "-"} (
                        {tx?.gasLimit && tx?.gasUsed
                          ? ((parseInt(tx.gasUsed) / parseInt(tx.gasLimit)) * 100).toFixed(2)
                          : "0"}
                        %)
                      </span>
                    }
                                              ...
                                              {isAddress ? (
                                                ...
                                                <div className="bg-zinc-100 dark:bg-zinc-800 p-2 rounded text-xs space-y-1">
                                                  {param.components.map((comp, compIdx) => {
                                                    const compIsAddress =
                                                      comp.type === "address" &&
                                                      comp.value.startsWith("0x") &&
                                                      comp.value.length === 42;
                                                    const compIsNumber =
                                                      (comp.type.startsWith("uint") ||
                                                        comp.type.startsWith("int")) &&
                                                      /^\d+$/.test(comp.value);
                                                    return (
                                                      <div
                                                        key={compIdx}
                                  ...
                                  {decodedEvent.params.map((param, paramIdx) => (
                                    <span key={paramIdx} className="text-sm">
                                      ...
                                      <span
                                        ...
                                        style={{ color: param.indexed ? "#3b82f6" : "#10b981" }}
                                      >
                              ...
                              <div>
                                ...
                                <div className="space-y-2">
                                  {log.topics.map((topic, topicIdx) => {
                                    ...
                                    const indexedParams =
                                      decodedEvent?.params.filter((p) => p.indexed) || [];
                                    const paramForTopic =
                                      topicIdx > 0 ? indexedParams[topicIdx - 1] : null;
                                        ...
                                        <div className="flex items-center gap-2 flex-1 min-w-0 flex-wrap">
                                          ...
                                          {paramForTopic && (
                                            <span className="text-xs px-2 py-0.5 rounded bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400">
                                              {paramForTopic.name}:{" "}
                                              {paramForTopic.type === "address"
                                                ? formatAddress(paramForTopic.value)
                                                : paramForTopic.value}
                                            </span>
                                          )}
                                        </div>
