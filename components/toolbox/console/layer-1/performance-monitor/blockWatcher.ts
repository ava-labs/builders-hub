...
export class BlockWatcher {
  ...
  private parseBlock(block: RpcBlock): BlockInfo {
    ...
    const timestampMs = block.timestampMilliseconds ? Number(BigInt(block.timestampMilliseconds)) : timestampSec * 1000;
  ...
  async start(startFromBlock: number, blockHistory: number) {
    ...
    try {
      ...
      const historicalStartBlock = Math.max(Number(latestBlock) - blockHistory, 1);
    ...
    while (this.isRunning) {
      try {
        ...
        const endBlock =
          currentBlockNumber + BigInt(maxBatchSize) < BigInt(lastBlock)
        ...
        if (error instanceof Error && error.message.includes('cannot query unfinalized data')) {
          await new Promise((resolve) => setTimeout(resolve, 1000));
        } else {
