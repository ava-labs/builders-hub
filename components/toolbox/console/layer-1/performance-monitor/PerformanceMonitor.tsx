...
export default function PerformanceMonitor() {
  ...
  const [blockTimestampsMs, setBlockTimestampsMs] = useState<Map<number, { timestamp: number; timestampMs: number }>>(
    new Map(),
  ); // block number -> {timestamp (sec), timestampMs}
  ...
  const avgTransactions =
    chartData.length > 0 ? chartData.reduce((sum, point) => sum + point.transactions, 0) / chartData.length : 0;
  const avgGas = chartData.length > 0 ? chartData.reduce((sum, point) => sum + point.gasUsed, 0) / chartData.length : 0;
  const avgBlocks =
    chartData.length > 0 ? chartData.reduce((sum, point) => sum + point.blockCount, 0) / chartData.length : 0;
    ...
    <motion.div className="space-y-6" variants={sectionContainer} initial="hidden" animate="visible">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Historical Blocks</label>
            <select
              value={blockHistory}
        ...
        <div className="flex gap-3 pt-2">
          <Button onClick={startMonitoring} disabled={!rpcUrl || isMonitoring}>
            <Play className="h-4 w-4 mr-2" />
            Start Monitoring
          </Button>
          <Button onClick={stopMonitoring} disabled={!isMonitoring} variant="secondary">
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                />
                <ReferenceLine y={avgTransactions} stroke="#22c55e" strokeDasharray="5 5" strokeOpacity={0.7} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Area
                  ...
                  isAnimationActive={false}
                />
                <ReferenceLine y={avgGas} stroke="#22c55e" strokeDasharray="5 5" strokeOpacity={0.7} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Area
                  ...
                  isAnimationActive={false}
                />
                <ReferenceLine y={avgBlocks} stroke="#22c55e" strokeDasharray="5 5" strokeOpacity={0.7} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
