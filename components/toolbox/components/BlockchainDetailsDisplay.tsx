...
export default function BlockchainDetailsDisplay({
    ...
    return (
      <div className="mt-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm">
        <div className="p-4 flex items-center justify-center">
          ...
          <span className="ml-2 text-xs text-zinc-500 dark:text-zinc-400">Loading details...</span>
        </div>
      </div>
    );
  }
  ...
  const blockchainData =
    blockchain || (subnet?.blockchains?.[0] ? { ...subnet.blockchains[0], isTestnet } : null);
      ...
      >
        <div className="flex items-center">
          ...
          <div className="flex items-center">
            ...
            <div className="flex items-center space-x-2 ml-3">{getNetworkBadges()}</div>
          </div>
        </div>
        {currentIsExpanded ? (
        ...
        <div className="px-4 pb-4 space-y-4 border-t border-zinc-100 dark:border-zinc-800">
          ...
          {isSubnetView && subnet && (
            <>
              ...
              {/* Subnet Owner */}
              {subnet.subnetOwnershipInfo?.addresses &&
                subnet.subnetOwnershipInfo.addresses.length > 0 && (
                  <div className="bg-zinc-50/50 dark:bg-zinc-800/30 rounded-lg p-3">
                    ...
                    <div className="text-xs text-zinc-500 dark:text-zinc-400 mt-2">
                      {subnet.subnetOwnershipInfo.threshold} of{" "}
                      {subnet.subnetOwnershipInfo.addresses.length} signatures required
                    </div>
                  </div>
                )}
              ...
              {subnet.isL1 && subnet.l1ValidatorManagerDetails && (
                <div className="bg-blue-50/50 dark:bg-blue-900/20 rounded-lg p-3 border border-blue-200 dark:border-blue-800">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      onClick={() =>
                        copyToClipboard(subnet.l1ValidatorManagerDetails?.contractAddress)
                      }
