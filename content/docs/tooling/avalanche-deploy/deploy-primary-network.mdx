...
This uses a **separate Terraform state** from the L1 deployment (`terraform/aws-primary-network/`), creating i4i.xlarge instances with 937GB NVMe drives.

Edit `terraform/aws-primary-network/terraform.tfvars` before running:
...
This runs playbook `10-deploy-primary-network.yml`, which:
...
Edit `terraform/aws-primary-network/terraform.tfvars`:
