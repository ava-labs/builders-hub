...
Poll the `avax.getAtomicTx` RPC with `txID` until the response includes `blockHeight`. Do not use `avax.getAtomicTxStatus`. It is deprecated, and the public API endpoints do not serve it after the Helicon upgrade.
