...
const metadata: ConsoleToolMetadata = {
  ...
  description:
    "Initialize the Native Token Staking Manager contract with the required configuration.",
...
function InitializeNativeStakingManager() {
  ...
  const {
    nativeStakingManagerAddress: storedStakingManagerAddress,
  ...
  async function handleInitialize() {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Failed to convert blockchain ID to hex: ${error instanceof Error ? error.message : "Unknown error"}`
        );
      }
      ...
      <Steps>
        <Step>
          ...
          <p className="text-sm text-gray-500">
            Choose the L1 subnet where the Native Token Staking Manager will be initialized. The
            Validator Manager address and blockchain ID will be automatically derived from your
            selection.
          </p>
          <SelectSubnet
          ...
          {subnetSelection.subnet && !subnetSelection.subnet.isL1 && (
            <div className="mt-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-md">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                <strong>Note:</strong> This subnet has not been converted to an L1 yet. Native Token
                Staking Manager can only be initialized for L1s.
              </p>
            </div>
          )}
        </Step>
        ...
        <Step>
          ...
          <p className="text-sm text-gray-500">
            ...
            <a href={INITIALIZE_FUNCTION_SOURCE_URL} target="_blank" rel="noreferrer">
              initialize()
            </a>{" "}
        ...
        <Step>
          ...
          <p className="text-sm text-gray-500">
            Set the Reward Calculator contract address that determines how rewards are distributed
            to validators. The Validator Manager address and Uptime Blockchain ID are automatically
            derived from your subnet selection.
          </p>
            ...
            {validatorManagerAddress && (
              <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-md">
                ...
                <p className="text-sm mt-1">
                  <strong>Uptime Blockchain ID (CB58):</strong>{" "}
                  <code>{blockchainId || "Loading..."}</code>
                </p>
                {blockchainId && (
                  <p className="text-sm mt-1">
                    <strong>Uptime Blockchain ID (Hex):</strong>{" "}
                    <code>
                      {(() => {
                        try {
        ...
        <Step>
          ...
          <p className="text-sm text-gray-500">
            Configure the staking parameters that define how validators and delegators can
