...
In the context of Avalanche, **[Avalanche L1s](/docs/avalanche-l1s)** are a special focus which requires new tooling and support for playing, working and testing with this unique feature of the Avalanche ecosystem.
...
`--node-config` allows to specify specific AvalancheGo config parameters to the new node. See [here](/docs/primary-network/configure/configs-flags) for the reference of supported flags.
...
For general Avalanche L1 documentation, please refer to [Avalanche L1s](/docs/avalanche-l1s). ANR can be a great helper working with Avalanche L1s, and can be used to develop and test new Avalanche L1s before deploying them in public networks. However, for a smooth and guided experience, we recommend using [Avalanche-CLI](/docs/tooling/create-deploy-avalanche-l1s/deploy-locally). These examples expect a basic understanding of what Avalanche L1s are and their usage.
