import { withApi, successResponse, ValidationError } from '@/lib/api';
import {
  queryClickHouse,
...
export const GET = withApi(async (req) => {
  ...
  if (!address || !/^0x[a-fA-F0-9]{40}$/.test(address)) {
    throw new ValidationError('Invalid or missing address parameter');
  }
  ...
  // Aggregate totals
  const totalGasReceived = callersResult.data.reduce((sum, r) => sum + (parseInt(r.gas) || 0), 0);
  const totalGasGiven = calleesResult.data.reduce((sum, r) => sum + (parseInt(r.gas) || 0), 0);
  const selfGas = Math.max(totalGasReceived - totalGasGiven, 0);
  const selfGasRatio = totalGasReceived > 0 ? selfGas / totalGasReceived : 0;

  const totalAvaxReceived = callersResult.data.reduce((sum, r) => sum + (r.avax || 0), 0);
  const totalAvaxGiven = calleesResult.data.reduce((sum, r) => sum + (r.avax || 0), 0);
