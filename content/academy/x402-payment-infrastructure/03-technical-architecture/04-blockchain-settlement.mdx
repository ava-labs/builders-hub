...
### What Facilitators Verify

When a transaction is submitted, facilitators check:

1. **Transaction Existence**: The transaction hash exists on the blockchain
2. **Transaction Success**: The transaction executed without errors
3. **Correct Recipient**: Funds were sent to the expected `payTo` address
4. **Correct Amount**: The transfer amount matches or exceeds `maxAmountRequired`
5. **Recent Timestamp**: The transaction was created recently (prevents replay of old transactions)
6. **Confirmation Status**: The transaction has reached finality
...
When selecting a blockchain for x402 payments, consider:

1. **Settlement Speed**: How fast do you need payment confirmation?
2. **Transaction Costs**: What are the gas fees on this chain?
3. **Facilitator Support**: Does your chosen facilitator support this chain?
4. **User Base**: Which chains do your users already have wallets and tokens on?

For most micropayment use cases, **Avalanche** and **Base** offer the best combination of speed (\~2 seconds) and cost (~\$0.001 gas).
...
**Next**: Learn about [Security Considerations](./05-security-considerations) including replay attack prevention, signature validation, and settlement monitoring.
