...
export const POST = withAuth(async (request: NextRequest, _context, session) => {
  try {
    ...
    if (!form_data?.build_games) {
      return NextResponse.json({ error: 'form_data.build_games is required' }, { status: 400 });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (existing) {
        ...
        const existingBuildGames = (existing.form_data as Record<string, unknown>)?.build_games ?? {};
        const mergedData: Prisma.InputJsonValue = {
          build_games: {
            ...
            stages:
              existingBuildGames && (existingBuildGames as Record<string, Prisma.InputJsonValue>).stages
