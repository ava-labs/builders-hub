...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (normalizedPath.startsWith("/docs/")) {
      ...
      page = integration.getPage(
        normalizedPath.slice("/integrations".length).split("/").filter(Boolean)
      );
      source = "integrations";
    ...
    if (!page) {
      return NextResponse.json({ error: `Page not found: ${normalizedPath}` }, { status: 404 });
    }
