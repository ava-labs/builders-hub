...
import { AuthModal } from "../ambassador-dao/sections/auth-modal";
...
const CertificatePage: React.FC<CertificatePageProps> = ({ courseId }) => {
  ...
  const generateCertificate = async () => {
    if (!userData) {
      toast.error("Please login to generate certificate");
  ...
  return (
    <div className='max-w-4xl mx-auto p-4'>
      {chapters.map((chapter) => {
        ...
        const incompleteQuizzes = chapterQuizzes.filter(
      ...
      {allQuizzesCompleted && (
        <div className='mt-12 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8'>
          ...
          <p className='text-center text-gray-600 dark:text-gray-300 mb-8'>
            You've completed all quizzes for the{" "}
            {quizData.courses[courseId].title} course. Claim your certificate
            now!
          </p>
          <div className='mb-6'>
            <label
      ...
      {!allQuizzesCompleted && (
        <div className='mt-12 bg-muted rounded-lg shadow-lg p-8'>
          ...
          Complete all quizzes to unlock your certificate and share your
          achievement!
        </div>
      )}
