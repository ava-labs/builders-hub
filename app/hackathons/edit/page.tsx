      ...
      <ul className="flex flex-wrap gap-2">
        {myHackathons.map((hackathon) => (
          <li
            ...
            title={
              isDevrel
                ? `${hackathon.title} (Created by: ${hackathon.created_by_name || "Unknown"})`
                : hackathon.title
            }
          >
            <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-3 h-3 rounded-full ${
                      hackathon.is_public ? "bg-green-500" : "bg-gray-400"
                    }`}
              ...
              {isDevrel && hackathon.created_by_name && (
                <span className="text-xs opacity-75">by {hackathon.created_by_name}</span>
              )}
            </div>
          </li>
        ))}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-lg p-6 max-w-4xl w-full max-h-[90vh] flex flex-col">
        <h2 className="text-lg font-bold mb-4 flex-shrink-0">
          {t[language].confirmUpdateTitle || "Confirm Update"}
        </h2>
        <p className="mb-2 flex-shrink-0">
          ...
          {fieldsToUpdate.map(({ key, oldValue, newValue }) => (
            <li key={key} className="mb-1">
              ...
              <div className="overflow-x-auto max-w-full border border-gray-200 dark:border-gray-700 rounded p-2">
                <div className="text-red-600 dark:text-red-500 line-through whitespace-nowrap text-sm min-w-max">
                  {String(oldValue)}
                </div>
              </div>
              <div className="overflow-x-auto max-w-full border border-gray-200 dark:border-gray-700 rounded p-2">
                <div className="text-green-600 dark:text-green-500 whitespace-nowrap text-sm min-w-max">
                  {String(newValue)}
                </div>
              </div>
            </li>
      ...
      {collapsed ? (
        ...
        <>
          ...
          <div className="mb-2 text-zinc-500 text-xs">
            Type a detailed description with formatting. Use the buttons below or type HTML
            directly.
          </div>
            ...
            <button
              ...
              onClick={() => {
                const textarea = document.querySelector(
                  `textarea[name="track-description-${index}"]`
                ...
                if (textarea) {
                  ...
                  const newText =
                    (rawTrackDescriptions[index] || "").substring(0, start) +
                    `<b>${selectedText}</b>` +
                    (rawTrackDescriptions[index] || "").substring(end);
    ...
    >
      ...
      {collapsed ? (
        ...
        <>
          ...
          <Select value={speaker.icon} onValueChange={(value) => onChange(index, "icon", value)}>
            <SelectTrigger className="mb-3">
              <SelectValue placeholder="Select Icon" />
            </SelectTrigger>
            <SelectContent>
    ...
    >
      ...
      {collapsed ? (
        ...
        <>
          ...
          <Select value={resource.icon} onValueChange={(value) => onChange(index, "icon", value)}>
            <SelectTrigger className="mb-3">
              <SelectValue placeholder="Select Icon" />
            </SelectTrigger>
            <SelectContent>
...
const HackathonsEdit = () => {
  ...
  const getMyHackathons = async () => {
    ...
    try {
      ...
      if (response.data?.hackathons?.length > 0) {
        ...
        const unfinishedHackathons = response.data.hackathons.filter((hackathon: Hackathon) => {
  ...
  const handleSelectHackathon = (hackathon: Hackathon) => {
    ...
    setFormDataContent({
      ...(hackathon.content ?? {}),
      tracks: hackathon.content?.tracks ?? [
        { icon: "", logo: "", name: "", partner: "", description: "", short_description: "" },
      ],
      address: hackathon.content?.address ?? "",
      partners: hackathon.content?.partners ?? [""],
      schedule: hackathon.content?.schedule ?? [
        { url: null, date: "", name: "", category: "", location: "", description: "", duration: 0 },
      ],
      speakers: (hackathon.content?.speakers ?? []).map((s: ISpeaker) => ({
        ...s,
        picture: s.picture ?? "",
      })),
    ...
    hackathon.content?.tracks?.forEach((track: ITrack, index: number) => {
  ...
  const [collapsedTracks, setCollapsedTracks] = useState<boolean[]>(
    formDataContent.tracks.map(() => false)
  );
  ...
  const [collapsedSchedules, setCollapsedSchedules] = useState<boolean[]>(
    formDataContent.schedule.map(() => false)
  );
  const [collapsedSpeakers, setCollapsedSpeakers] = useState<boolean[]>(
    formDataContent.speakers.map(() => false)
  );
  const [collapsedResources, setCollapsedResources] = useState<boolean[]>(
    formDataContent.resources.map(() => false)
  );
  ...
  const addSpeaker = () => {
    setFormDataContent({
      ...formDataContent,
      speakers: [...formDataContent.speakers, { icon: "", name: "", category: "", picture: "" }],
    });
  };
  ...
  const addResource = () => {
    setFormDataContent({
      ...formDataContent,
      resources: [...formDataContent.resources, { icon: "", link: "", title: "", description: "" }],
    });
  };
  ...
  const [fieldsToUpdate, setFieldsToUpdate] = useState<FieldChange[]>([]);
    ...
    if (processedData.content?.speakers) {
      for (let i = 0; i < processedData.content.speakers.length; i++) {
        ...
        if (speaker.picture && speaker.picture.startsWith("data:image/")) {
          ...
          try {
            processedData.content.speakers[i].picture = await uploadBase64ToVercel(
              speaker.picture,
              fileName
            );
            console.log(
              `Uploaded speaker ${i} picture to Vercel storage:`,
              processedData.content.speakers[i].picture
            );
          } catch (error) {
  ...
  const doSubmit = async () => {
    setLoading(true);
    let dataToSend;
    if (isSelectedHackathon) dataToSend = { ...getDataToSend(), updated_by: session?.user?.id };
    else {
    ...
    try {
      ...
      if (response.ok) {
        setMyHackathons((prev) =>
          prev.map((hackathon) =>
            hackathon.id === hackathonId ? { ...hackathon, is_public: isPublic } : hackathon
          )
        );

        if (selectedHackathon?.id === hackathonId) {
          setSelectedHackathon((prev: Hackathon | null) =>
        ...
        console.log(
          `Hackathon ${hackathonId} visibility updated to ${isPublic ? "public" : "private"}`
        );
      } else {
  ...
  const handleResourceFieldChange = useCallback(
    (idx: number, field: string, value: any) => {
      setFormDataContent((prev) => {
  ...
  const loadMockData = () => {
    setFormDataMain({
      ...
      description:
        "Build the future of Web3 on Avalanche. Join us for an exciting hackathon where we will create innovative blockchain solutions.",
    ...
    setFormDataLatest({
      ...
      banner:
        "https://qizat5l3bwvomkny.public.blob.vercel-storage.com/builders-hub/hackathon-images/Avalanche%20Chile/bannerchilehor.png",
      ...
      small_banner:
        "https://qizat5l3bwvomkny.public.blob.vercel-storage.com/builders-hub/hackathon-images/Avalanche%20Chile/bannerchile.png",
    });
    ...
    setFormDataContent({
      tracks: [
        {
          ...
          partner: "Avalanche",
          description:
            "<b>DeFi Innovation:</b> Build the next generation of decentralized finance protocols. Create lending platforms, DEXs, yield farming strategies, or novel DeFi primitives that leverage Avalanche's high-speed, low-cost infrastructure.",
        ...
        },
        {
          ...
          partner: "Avalanche",
          description:
            "<b>Security & Privacy:</b> Develop cutting-edge security solutions and privacy-preserving technologies. Build secure wallets, privacy protocols, or tools that enhance the security and privacy of blockchain applications.",
        ...
        },
        {
          ...
          partner: "Avalanche",
          description:
            "<b>Gaming & NFTs:</b> Revolutionize gaming with blockchain technology. Create play-to-earn games, NFT marketplaces, gaming infrastructure, or tools that bridge traditional gaming with Web3.",
      ...
      schedule: [
        {
          ...
          location: "Main Stage",
          description:
            "Welcome to Avalanche Hackathon 2025! Join us for an inspiring opening ceremony with keynote speakers from the Avalanche ecosystem.",
          duration: 60,
        },
        {
          url: null,
          ...
          description:
            "Meet other participants, form teams, and start brainstorming your hackathon project ideas.",
          duration: 90,
        },
        {
          url: null,
          ...
          description:
            "Learn the fundamentals of building on Avalanche, including smart contract development and deployment.",
          duration: 120,
        },
        {
          url: null,
          ...
          description:
            "Get guidance from industry experts and experienced developers on your project.",
          duration: 180,
        },
        {
          url: null,
          ...
          location: "Main Stage",
          description:
            "Present your projects to judges and the community. Showcase your innovative solutions built on Avalanche.",
          duration: 240,
        },
      ],
      speakers: [
        {
          ...
          picture:
            "https://qizat5l3bwvomkny.public.blob.vercel-storage.com/builders-hub/hackathon-images/2259ff3def815083bf765c53d57327dc-1657109283036.jpg",
        },
      ],
      resources: [
        {
      ...
      speakers_text:
        "Students will have access to the Avalanche Academy curriculum, as well as Avalanche documentation and the Avalanche faucet.",
      join_custom_link: "",
      join_custom_text: "Join now",
      judging_guidelines:
        "Projects will be evaluated based on innovation, technical implementation, user experience, and potential impact.",
      ...
      <div className="bg-zinc-900 border-b border-zinc-700 p-4">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <LanguageButton language={language} onLanguageChange={setLanguage} t={t} />
          </div>
          <div className="flex items-center gap-4">
            <button
              ...
              <div className="flex gap-2 mb-4 sticky top-0 z-10 bg-zinc-950 py-2">
                ...
                {session?.user?.custom_attributes?.includes("devrel") && (
                  <Button
                    ...
                    onClick={() =>
                      handleToggleVisibility(selectedHackathon.id, !formDataMain.is_public)
                    }
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="bg-zinc-900/60 border border-zinc-700 rounded-lg p-6 my-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {collapsed.main && (
                        <button
                          onClick={() => setCollapsed({ ...collapsed, main: false })}
                          className="flex items-center gap-1 text-zinc-400 hover:text-red-500 cursor-pointer"
                        >
                          <ChevronRight className="w-5 h-5" /> {t[language].expand}
                        </button>
                      ...
                      <>
                        <div className="mb-4 p-4 bg-green-900/20 border border-green-500/30 rounded-lg">
                          ...
                          <p className="text-sm text-green-200">
                            Let's start with the basic information that will appear in your
                            hackathon preview.
                          </p>
                        </div>
                        ...
                        <div className="flex flex-col space-y-2 bg-zinc-900/60 border border-zinc-700 rounded-lg p-4 my-4">
                          ...
                          <div className="flex flex-wrap gap-2 items-center">
                            ...
                            <button
                              type="button"
                              onClick={addTag}
                              className="text-green-500 hover:text-green-700 px-2 py-1 border border-green-500 rounded"
                            >
                              + Add Tag
                            </button>
                          </div>
                  ...
                  <div className="bg-zinc-900/60 border border-zinc-700 rounded-lg p-6 my-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {collapsed.images && (
                        <button
                          onClick={() => setCollapsed({ ...collapsed, images: false })}
                          className="flex items-center gap-1 text-zinc-400 hover:text-red-500 cursor-pointer"
                        >
                          <ChevronRight className="w-5 h-5" /> {t[language].expand}
                        </button>
                      ...
                      <>
                        <div className="mb-4 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                          <h3 className="text-lg font-semibold text-blue-300 mb-2">
                            Hackathon Images & Branding
                          </h3>
                          <p className="text-sm text-blue-200">
                            Upload your hackathon banner, icon, and small banner. Images will be
                            stored locally and uploaded to the database when you submit the form.
                          </p>
                        </div>
                        ...
                        <div className="mb-6">
                          ...
                          <div className="mb-2 text-zinc-400 text-sm">
                            The main banner image displayed at the top of your hackathon page
                          </div>
                          ...
                          <div className="mb-4">
                            ...
                            {formDataLatest.banner && (
                              <div className="mt-4">
                                ...
                                <div
                                  className="relative w-full max-w-2xl mx-auto bg-zinc-800 border border-zinc-600 rounded-lg overflow-hidden"
                                  style={{ aspectRatio: "1600/909" }}
                                >
                                  <Image
                  ...
                  */}
                        ...
                        <div className="mb-6">
                          ...
                          <div className="mb-2 text-zinc-400 text-sm">
                            A smaller banner image for additional branding
                          </div>
                          ...
                          <div className="mb-4">
                            ...
                            {formDataLatest.small_banner && (
                              <div className="mt-4">
                                ...
                                <div
                                  className="relative w-32 mx-auto bg-zinc-800 border border-zinc-600 rounded-lg overflow-hidden"
                                  style={{ aspectRatio: "601/1028" }}
                                >
                                  <Image
                  ...
                  <div className="bg-zinc-900/60 border border-zinc-700 rounded-lg p-6 my-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {collapsed.about && (
                        <button
                          onClick={() => setCollapsed({ ...collapsed, about: false })}
                          className="flex items-center gap-1 text-zinc-400 hover:text-red-500 cursor-pointer"
                        >
                          <ChevronRight className="w-5 h-5" /> {t[language].expand}
                        </button>
                      ...
                      <>
                        <div className="mb-4 p-4 bg-orange-900/20 border border-orange-500/30 rounded-lg">
                          <h3 className="text-lg font-semibold text-orange-300 mb-2">
                            Participants & Prize Information
                          </h3>
                          <p className="text-sm text-orange-200">
                            Now let's add details about participants and the prize pool.
                          </p>
                        </div>
                  ...
                  */}
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            setFormDataMain((prev) => ({
                              ...prev,
                              total_prizes: Number(e.target.value) || 0,
                            }));
                  ...
                  <div className="bg-zinc-900/60 border border-zinc-700 rounded-lg p-6 my-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {collapsed.trackText && (
                        <button
                          onClick={() => setCollapsed({ ...collapsed, trackText: false })}
                          className="flex items-center gap-1 text-zinc-400 hover:text-red-500 cursor-pointer"
                        >
                          <ChevronRight className="w-5 h-5" /> {t[language].expand}
                        </button>
                      ...
                      <>
                        <div className="mb-4 p-4 bg-purple-900/20 border border-purple-500/30 rounded-lg">
                          ...
                          <p className="text-sm text-purple-200">
                            Write detailed information about your hackathon tracks, program
                            structure, and timeline. Use paragraphs and line breaks - they will be
                        ...
                        <div className="mb-2 text-zinc-500 text-xs">
                          Write a step-by-step schedule outlining what will happen, either hour by
                          hour or week by week. Use the formatting buttons below or type markdown
                          directly.
                        </div>
                  ...
                  <div className="bg-zinc-900/60 border border-zinc-700 rounded-lg p-6 my-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {collapsed.content && (
                        <button
                          onClick={() => setCollapsed({ ...collapsed, content: false })}
                          className="flex items-center gap-1 text-zinc-400 hover:text-red-500 cursor-pointer"
                        >
                          <ChevronRight className="w-5 h-5" /> {t[language].expand}
                        </button>
                      ...
                      <>
                        ...
                        <div className="space-y-4">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setFormDataContent({ ...formDataContent, address: e.target.value })
                            }
                            className="w-full mb-4"
                            required
                          />
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <div className="mb-2 text-zinc-400 text-sm">
                              {t[language].submissionDeadlineHelp}
                            </div>
                            <Input
                              type="datetime-local"
                  ...
                  <div className="bg-zinc-900/60 border border-zinc-700 rounded-lg p-6 my-6 mt-10">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {collapsed.last && (
                        <button
                          onClick={() => setCollapsed({ ...collapsed, last: false })}
                          className="flex items-center gap-1 text-zinc-400 hover:text-red-500 cursor-pointer"
                        >
                          <ChevronRight className="w-5 h-5" /> {t[language].expand}
                        </button>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setFormDataLatest({ ...formDataLatest, start_date: e.target.value })
                              }
                              className="w-full mb-4"
                              required
                            />
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setFormDataLatest({ ...formDataLatest, end_date: e.target.value })
                              }
                              className="w-full mb-4"
                              required
                            />
                          ...
                          <div>
                            ...
                            <div className="mb-2 text-zinc-400 text-sm">
                              {t[language].timezoneHelp}
                            </div>
                            <Select
                              value={formDataLatest.timezone}
                              onValueChange={(value) =>
                                setFormDataLatest({ ...formDataLatest, timezone: value })
                              }
                            >
                              ...
                              <SelectContent>
                                <SelectItem value="America/New_York">
                                  New York (EST/EDT) - GMT-5/-4
                                </SelectItem>
                                <SelectItem value="America/Chicago">
                                  Chicago (CST/CDT) - GMT-6/-5
                                </SelectItem>
                                <SelectItem value="America/Denver">
                                  Denver (MST/MDT) - GMT-7/-6
                                </SelectItem>
                                <SelectItem value="America/Los_Angeles">
                                  Los Angeles (PST/PDT) - GMT-8/-7
                                </SelectItem>
                                <SelectItem value="America/Toronto">
                                  Toronto (EST/EDT) - GMT-5/-4
                                </SelectItem>
                                <SelectItem value="America/Vancouver">
                                  Vancouver (PST/PDT) - GMT-8/-7
                                </SelectItem>
                                <SelectItem value="America/Mexico_City">
                                  Mexico City (CST/CDT) - GMT-6/-5
                                </SelectItem>
                                <SelectItem value="America/Bogota">
                                  Bogotá, Colombia (COT) - GMT-5
                                </SelectItem>
                                <SelectItem value="America/Costa_Rica">
                                  San José, Costa Rica (CST) - GMT-6
                                </SelectItem>
                                <SelectItem value="America/Panama">
                                  Panama City, Panama (EST) - GMT-5
                                </SelectItem>
                                <SelectItem value="America/Caracas">
                                  Caracas, Venezuela (VET) - GMT-4
                                </SelectItem>
                                <SelectItem value="America/La_Paz">
                                  La Paz, Bolivia (BOT) - GMT-4
                                </SelectItem>
                                <SelectItem value="America/Lima">
                                  Lima, Peru (PET) - GMT-5
                                </SelectItem>
                                <SelectItem value="America/Sao_Paulo">
                                  São Paulo, Brazil (BRT) - GMT-3
                                </SelectItem>
                                <SelectItem value="America/Santiago">
                                  Santiago, Chile (CLT) - GMT-3
                                </SelectItem>
                                <SelectItem value="America/Buenos_Aires">
                                  Buenos Aires, Argentina (ART) - GMT-3
                                </SelectItem>
                                <SelectItem value="Europe/London">
                                  London (GMT/BST) - GMT+0/+1
                                </SelectItem>
                                <SelectItem value="Europe/Paris">
                                  Paris (CET/CEST) - GMT+1/+2
                                </SelectItem>
                                <SelectItem value="Europe/Berlin">
                                  Berlin (CET/CEST) - GMT+1/+2
                                </SelectItem>
                                <SelectItem value="Europe/Rome">
                                  Rome (CET/CEST) - GMT+1/+2
                                </SelectItem>
                                <SelectItem value="Europe/Madrid">
                                  Madrid (CET/CEST) - GMT+1/+2
                                </SelectItem>
                                <SelectItem value="Europe/Amsterdam">
                                  Amsterdam (CET/CEST) - GMT+1/+2
                                </SelectItem>
                                <SelectItem value="Europe/Zurich">
                                  Zurich (CET/CEST) - GMT+1/+2
                                </SelectItem>
                                <SelectItem value="Europe/Stockholm">
                                  Stockholm (CET/CEST) - GMT+1/+2
                                </SelectItem>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-lg p-6 max-w-lg w-full">
                  ...
                  <p className="mb-4">
                    ...
                    <br />
                    Hackathon: <span className="font-semibold">{selectedHackathon?.title}</span>
                  </p>
                  <div className="flex justify-end gap-2">
                    <button
                      onClick={() => setShowDeleteModal(false)}
                      className="px-4 py-2 rounded bg-zinc-200 dark:bg-zinc-700 hover:bg-zinc-300 dark:hover:bg-zinc-600 cursor-pointer"
                    >
                      Cancel
                    </button>
                    <button
            ...
            <HackathonPreview
              hackathonData={{
                ...
                content: {
                  ...
                  submission_custom_link: formDataContent.submission_custom_link
                    ? formDataContent.submission_custom_link
                    : undefined,
