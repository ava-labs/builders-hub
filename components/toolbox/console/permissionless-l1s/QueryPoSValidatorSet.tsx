...
export default function QueryPoSValidatorSet() {
  ...
  const calculateValidatorTimeRemaining = (
    startTime: bigint,
    minStakeDuration: bigint,
  ): { remaining: string; canBeRemoved: boolean } => {
    ...
    try {
      ...
      if (!rpcUrl && validatorManagerDetails.blockchainId) {
        ...
        if (l1) {
          ...
          setRpcUrlError(
            `L1 not found in your network list. Please add this L1 to your networks first (Blockchain ID: ${validatorManagerDetails.blockchainId})`,
          );
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (activeTab === 'validators') {
      ...
      const filtered = enrichedValidators.filter(
        (validator) =>
                            ...
                            <td className="px-3 py-3 whitespace-nowrap text-sm text-zinc-800 dark:text-zinc-200">
                              <span className="font-medium text-blue-600 dark:text-blue-400">
                                {validator.stakeValue
                                  ? `${parseFloat(validator.stakeValue).toFixed(2)} tokens`
                            ...
                            {isPoSL1 && (
                              <td className="px-3 py-3 whitespace-nowrap text-sm text-zinc-800 dark:text-zinc-200">
                                <div className="flex items-center">
                                  ...
                                  <span>
                                    {validator.posInfo ? (validator.posInfo.delegationFeeBips / 100).toFixed(2) : '-'}%
                                  </span>
                                </div>
                              </td>
                            )}
                            ...
                            <td className="px-3 py-3 whitespace-nowrap text-sm">
                              ...
                              >
                                {validator.validatorInfo
                                  ? ValidatorStatusNames[validator.validatorInfo.status]
                                  : 'Active'}
                              </span>
                            </td>
            ...
            <>
              {isLoadingDelegations ? (
                        ...
                        {filteredDelegations.map((delegation, index) => (
                          ...
                          >
                            <td className="px-3 py-3 text-sm font-mono truncate max-w-[120px] text-zinc-800 dark:text-zinc-200">
                              <div className="flex items-center">
                                <span title={delegation.delegationID} className="truncate">
                                  {delegation.delegationID.substring(0, 10)}...
                                </span>
                                <button
                            ...
                            <td className="px-3 py-3 text-sm font-mono truncate max-w-[140px] text-zinc-800 dark:text-zinc-200">
                              <span title={delegation.nodeId || delegation.validationID}>
                                {delegation.nodeId
                                  ? delegation.nodeId.substring(0, 12) + '...'
                                  : delegation.validationID.substring(0, 10) + '...'}
                              </span>
                            </td>
                            <td className="px-3 py-3 whitespace-nowrap text-sm text-zinc-800 dark:text-zinc-200">
                              <span className="font-medium text-blue-600 dark:text-blue-400">
                                {delegation.stakeValue
                                  ? `${parseFloat(delegation.stakeValue).toFixed(4)} tokens`
                                  : formatWeight(delegation.weight)}
                              </span>
                            </td>
                            <td className="px-3 py-3 whitespace-nowrap text-sm">
                ...
                <div className="text-center py-8 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
                  ...
                  <p className="text-zinc-500 dark:text-zinc-500 text-xs">
                    You don&apos;t have any delegations on this L1
                  </p>
                </div>
              )}
            </>
              ...
              <div className="space-y-3">
                <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                  ...
                  <div className="flex items-center">
                    ...
                    >
                      {copiedId === selectedValidator.nodeId ? (
                        <Check size={12} className="text-green-500" />
                      ) : (
                ...
                <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                  ...
                  <div className="flex items-center">
                    ...
                    <button
                      ...
                      className="ml-1 p-0.5 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
                    >
                      {copiedId === selectedValidator.validationId ? (
                        <Check size={12} className="text-green-500" />
                      ) : (
                        <Copy size={12} className="text-zinc-500" />
                      )}
                ...
                {selectedValidator.posInfo?.owner && (
                  <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                    ...
                    <div className="flex items-center">
                      ...
                      <button
                        onClick={() => copyToClipboard(selectedValidator.posInfo!.owner)}
                        className="ml-1 p-0.5 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
                      >
                        {copiedId === selectedValidator.posInfo.owner ? (
                          <Check size={12} className="text-green-500" />
                        ) : (
                          <Copy size={12} className="text-zinc-500" />
                        )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                    ...
                    <p className="font-mono text-sm font-semibold text-blue-600 dark:text-blue-400">
                      {selectedValidator.stakeValue
                        ? `${parseFloat(selectedValidator.stakeValue).toFixed(4)}`
                        : formatWeight(selectedValidator.weight)}
                    </p>
                  </div>
                  <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
              ...
              <div className="bg-zinc-50 dark:bg-zinc-800/70 p-4 rounded-lg border border-zinc-200 dark:border-zinc-700">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                    ...
                    {selectedValidator.isGenesisValidator ? (
                      ...
                      <p
                        className={`font-medium text-sm ${selectedValidator.canBeRemoved ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400'}`}
                      >
                ...
                {selectedValidator.posInfo?.uptimeSeconds !== undefined && (
                  <div className="mt-3 p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                    ...
                    <p className="font-medium text-sm text-zinc-800 dark:text-zinc-200">
                      {Math.floor(Number(selectedValidator.posInfo.uptimeSeconds) / 3600)}h{' '}
                      {Math.floor((Number(selectedValidator.posInfo.uptimeSeconds) % 3600) / 60)}m
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                  ...
                  <div className="flex items-center">
                    ...
                    <button
                      onClick={() => copyToClipboard(selectedDelegation.delegationID)}
                      className="ml-1 p-0.5 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
                    >
                      {copiedId === selectedDelegation.delegationID ? (
                        <Check size={12} className="text-green-500" />
                      ) : (
                        <Copy size={12} className="text-zinc-500" />
                      )}
            ...
            <div className="bg-zinc-50 dark:bg-zinc-800/70 p-4 rounded-lg border border-zinc-200 dark:border-zinc-700">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                  ...
                  <p className="font-mono text-sm font-semibold text-blue-600 dark:text-blue-400">
                    {selectedDelegation.stakeValue
                      ? parseFloat(selectedDelegation.stakeValue).toFixed(4)
                      : formatWeight(selectedDelegation.weight)}
                  </p>
                </div>
                <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                  <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">Status</p>
                  <p
                    className={`font-medium text-sm ${selectedDelegation.status === 2 ? 'text-green-600' : 'text-zinc-600'}`}
                  >
                ...
                <div className="p-2.5 bg-white dark:bg-zinc-900/80 rounded-md border border-zinc-200 dark:border-zinc-700">
                  <p className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">Time Remaining</p>
                  <p
                    className={`font-medium text-sm ${selectedDelegation.canBeRemoved ? 'text-green-600' : 'text-yellow-600'}`}
                  >
