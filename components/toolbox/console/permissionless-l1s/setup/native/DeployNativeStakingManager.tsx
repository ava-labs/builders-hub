...
function calculateLibraryHash(libraryPath: string) {
  const hash = keccak256(new TextEncoder().encode(libraryPath)).slice(2);
...
function DeployNativeStakingManager() {
  ...
  const getLinkedBytecode = () => {
    if (!validatorMessagesLibAddress) {
      throw new Error(
        "ValidatorMessages library must be deployed first. Please deploy it in the Validator Manager setup."
      );
    }
  ...
  async function deployNativeTokenStakingManager() {
    ...
    try {
      ...
      if (!validatorMessagesLibAddress) {
        throw new Error(
          "ValidatorMessages library must be deployed first. Please go to Validator Manager Setup and deploy the library."
        );
      }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500">
        This will deploy the <code>NativeTokenStakingManager</code> contract to the EVM network{" "}
        <code>{viemChain?.id}</code>. The Native Token Staking Manager enables permissionless
        staking on your L1 using the native token.
      </p>
      <p className="text-sm text-gray-500">
        Contract source:{" "}
        <a href={NATIVE_TOKEN_STAKING_MANAGER_SOURCE_URL} target="_blank" rel="noreferrer">
      ...
      {!validatorMessagesLibAddress ? (
        <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded-md">
          <p className="text-sm text-red-800 dark:text-red-200">
            <strong>Required:</strong> ValidatorMessages library must be deployed first. Please go
            to the <strong>Validator Manager Setup</strong> section and deploy the ValidatorMessages
            library.
          </p>
        </div>
      ) : (
        <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-md">
          <p className="text-sm text-green-800 dark:text-green-200">
            <strong>Ready:</strong> ValidatorMessages library found at:{" "}
            <code>{validatorMessagesLibAddress}</code>
          </p>
        </div>
      )}
      ...
      <p>
        Deployment Status: <code>{nativeStakingManagerAddress || "Not deployed"}</code>
      </p>

      {nativeStakingManagerAddress && (
        <Success label="Native Token Staking Manager Address" value={nativeStakingManagerAddress} />
      )}
    </div>
  );
}
