...
When a chain starts, it must bootstrap to catch up with the network:

<Mermaid chart={`
stateDiagram-v2
    [*] --> Bootstrapping
    Bootstrapping --> StateSyncing: State sync enabled
    StateSyncing --> NormalOperation: Sync complete
    Bootstrapping --> NormalOperation: Bootstrap complete
    NormalOperation --> [*]
`} />
