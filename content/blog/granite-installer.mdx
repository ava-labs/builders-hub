...
Use the Docker image that bundles AvalancheGo together with the Subnet-EVM plugin:

- **Docker Image**: [`avaplatform/subnet-evm:v1.14.2`](https://hub.docker.com/r/avaplatform/subnet-evm/tags?name=v1.14.2)

```bash
docker pull avaplatform/subnet-evm:v1.14.2
```

<Callout type="info">
This Docker image includes the Subnet-EVM plugin pre-installed in `/avalanchego/build/plugins/`, so no additional plugin installation is needed. The image tag matches the AvalancheGo version it ships with.
</Callout>

<Callout type="warn">
The legacy `avaplatform/subnet-evm_avalanchego:<subnet-evm>_<avalanchego>` image (e.g. `v0.8.0_v1.14.0`) is deprecated. New AvalancheGo releases (v1.14.1+) are only published under `avaplatform/subnet-evm:<avalanchego-version>`.
</Callout>
