...
async function lookupTransactionAcrossChains(
  txHash: string
            ...
            <div className="space-y-4 sm:space-y-6 flex-1">
              <div>
                <div className="flex items-center gap-2 sm:gap-3 mb-3 flex-wrap">
                  ...
                  {chainsWithRpc.length > 0 && (
                    <div className="flex items-center gap-2 ml-2">
                      ...
                      {(l1ChainsData as L1Chain[]).filter((c) => c.rpcUrl).length -
                        chainsWithRpc.length >
                        0 && (
                        <span className="text-xs text-zinc-400">
                          +
                          {(l1ChainsData as L1Chain[]).filter(
                            (c) => c.rpcUrl && c.isTestnet !== true
                          ).length - chainsWithRpc.length}{" "}
                          more
                        </span>
                      )}
                    </div>
                ...
                <p className="text-sm sm:text-base text-zinc-500 dark:text-zinc-400 max-w-2xl mt-3">
                  Real-time activity across all Avalanche L1 chains - blocks, transactions, and
                  cross-chain messages from the entire ecosystem.
                </p>
