...
description: Understanding active state vs archival state in EVM chains, and how to manage disk usage through state sync and offline pruning.
---

When running an EVM-based blockchain (C-Chain or Subnet-EVM L1s), your node stores blockchain state on disk. Understanding the difference between **active state**, **active state with state sync snapshots**, and **archival state** is crucial for managing disk space and choosing the right sync method.

## State Types Comparison

Your node's storage requirements depend on which type of state you're maintaining:

| Property | Active State | Active State with State Sync Snapshots | Archival State |
|----------|--------------|----------------------------------------|----------------|
| **Size (C-Chain)** | ~500 GB | ~750 GB - 1 TB | ~3 TB+ (and growing) |
| **Contents** | Current account balances, contract storage, code | Current state + recent historical state sync snapshots | Complete state history at every block |
...
**Active state with state sync snapshots** is what your node accumulates over time after starting with active state. As blocks are processed, state sync snapshots are retained for serving other nodes that want to bootstrap via state sync. This causes disk usage to grow beyond the active state size. Running offline pruning (state sync snapshot deletion) periodically removes these snapshots while keeping your node functional. Most long-running validators operate in this state.

### Archival State (Total State)
...
<Callout>
Most validators and RPC nodes only need **active state** or can operate with **active state with state sync snapshots**. Archive nodes are specialized infrastructure for historical data access.
</Callout>
...
- **Active State with State Sync Snapshots** (red line): Represents typical validator disk usage over time
  - Grows at a moderate rate as state sync snapshots accumulate
  - This is what most long-running validators experience without maintenance
...
- **Active State with periodic snapshot deletion** (green line): Shows nodes performing manual offline pruning periodically
  - After reaching active state, the node accumulates state sync snapshots over time
  - When offline pruning is performed, state drops back to active state level (node goes offline during this operation)
  - This sawtooth pattern represents the recommended maintenance cycle for validators
...
- **New validator (State Sync)**: When a new validator joins (blue line) via state sync, it quickly reaches the active state level by downloading the current state from peers
- **Active State** (black line): The minimal state required for validation, growing slowly over time
- **Green line**: After bootstrapping, the node accumulates state sync snapshots. Periodic offline pruning brings the state back down to active state level (the node goes offline during this maintenance operation)
...
| Running a block explorer or indexer | Keep archival state, add more storage |
