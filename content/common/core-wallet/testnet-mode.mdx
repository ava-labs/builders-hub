import Gallery from '@/components/content-design/gallery';

Since we do not want to play with real funds, we will switch to the Fuji Testnet, an exact replica of the Avalanche's Mainnet architecture with the difference that its tokens are valueless.

To switch to Testnet Mode, open the Core browser extension, click on the Tools icon in the sidebar, then select Settings, and finally activate Testnet Mode.

<Gallery url1="/common-images/core-wallet/go-to-menu.png" url2="/common-images/core-wallet/go-to-advanced.png" />