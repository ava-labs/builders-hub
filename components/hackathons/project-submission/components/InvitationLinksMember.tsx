      ...
      {invitationResult?.InviteLinks?.map((link: any) => (
        <li key={link.User}>
          <button
            onClick={async () => {
              try {
                await navigator.clipboard.writeText(link.Invitation);
          ...
          >
            <span className="font-medium text-gray-900 dark:text-gray-100">{link.User}</span>
            <span className="text-sm text-gray-500 dark:text-gray-400">Click to copy link</span>
          </button>
        </li>
      ))}
    </ul>
