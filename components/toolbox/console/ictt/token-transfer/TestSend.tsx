...
export default function TokenBridge() {
  ...
  const [sourceTransferrerType, setSourceTransferrerType] = useState<
    'unknown' | 'home' | 'nativeRemote' | 'erc20Remote'
  >('unknown');
  ...
  useEffect(() => {
    ...
    destToolboxStore?.nativeTokenHomeAddress,
    destToolboxStore?.erc20TokenHomeAddress,
    destL1?.id,
  ]);
      ...
      try {
        ...
        if (direction === 'source') {
          ...
          if (fetchedTokenAddress) {
            ...
            const tokenHomeAddr = (await publicClient
              .readContract({
                address: address,
            ...
            if (tokenHomeAddr) {
              ...
              if (isNativeRemote) {
                ...
                const [d, n, s] = await Promise.all([
                  publicClient
                    .readContract({ address, abi: NativeTokenRemoteABI.abi, functionName: 'decimals' })
                    .catch(() => null),
                  publicClient
                    .readContract({ address, abi: NativeTokenRemoteABI.abi, functionName: 'name' })
                    .catch(() => null),
                  publicClient
                    .readContract({ address, abi: NativeTokenRemoteABI.abi, functionName: 'symbol' })
                    .catch(() => null),
                ]);
        ...
        const [fetchedDecimals, fetchedName, fetchedSymbol, fetchedBalance, fetchedAllowance] = await Promise.all([
          tokenDecimalsLocal !== null
            ? Promise.resolve(BigInt(tokenDecimalsLocal))
        ...
        const token: Token = {
          address: tokenAddress,
          name: (isWrapped === true && detectedSourceType !== 'nativeRemote'
            ? selectedL1?.coinName
            : fetchedName) as string,
          symbol: (isWrapped === true && detectedSourceType !== 'nativeRemote'
            ? selectedL1?.coinName
            : fetchedSymbol) as string,
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const { request } = await publicClient.simulateContract({
        ...
        abi:
          sourceTransferrerType === 'nativeRemote'
      ...
      if (receipt.status === 'success') {
        let log = receipt.logs.find(
          (l: any) => l.address.toLowerCase() === '0x253b2784c75e510dD0fF1da844684a1aC0aa5fcf'.toLowerCase(),
        );
