---
title: Bridge Tokens
description: Transfer USDC from Fuji to your L1 as native gas tokens.
updated: 2025-12-17
authors: [nicolasarnedo]
icon: Terminal
---

import { Step, Steps } from 'fumadocs-ui/components/steps';
import { Callout } from 'fumadocs-ui/components/callout';
import ToolboxMdxWrapper from "@/components/toolbox/academy/wrapper/ToolboxMdxWrapper.tsx";
import TestSend from "@/components/toolbox/console/ictt/token-transfer/TestSend";

# Bridge USDC to Native Tokens

With your bridge fully configured and collateralized, you can now transfer USDC from Fuji to your L1, where it becomes **native gas tokens**.

<Steps>
<Step>
### Connect to Fuji C-Chain

Switch your wallet to **Fuji C-Chain** (the source chain).

<Callout type="info">
You need **USDC** on Fuji for the amount you want to bridge, **plus AVAX** for gas.
</Callout>

</Step>

<Step>
### Bridge using the Token Transfer tool

Use the tool below to initiate the cross-chain transfer:

1. Select your **ERC20TokenHome** as the source bridge contract
2. Select your **NativeTokenRemote** as the destination bridge contract
3. Enter the amount of USDC to bridge

<Callout type="warning">
Make sure you have:
1. Successfully registered and collateralized your bridge
2. USDC tokens in your wallet to bridge
3. AVAX for gas fees on Fuji
</Callout>

<ToolboxMdxWrapper enforceChainId={43113}>
    <TestSend />
</ToolboxMdxWrapper>

</Step>

<Step>
### Switch to your L1 and confirm

Switch your wallet to **your L1** and confirm your native token balance increased.

The bridged USDC is now usable as **native gas tokens** on your L1!

</Step>
</Steps>

## What Happened

1. USDC was **locked** in the ERC20TokenHome on Fuji
2. An ICM message was sent to your L1 via the relayer
3. NativeTokenRemote called the **Native Minter precompile** to mint native tokens
4. Native tokens were credited to your address

## Bridging Back

To bridge native tokens back to USDC:

1. Switch to your L1
2. Use the Token Transfer tool with reversed source/destination
3. Native tokens will be burned and USDC released on Fuji

## Summary

You've successfully:

- ✅ Created an L1 with USDC as the native gas token
- ✅ Deployed and connected bridge contracts
- ✅ Collateralized the bridge
- ✅ Bridged USDC → Native tokens

## Next Chapter

In the next chapter, you'll do the same flow but with **native AVAX** as the source token instead of an ERC-20.

