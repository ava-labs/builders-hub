...
description: Fireblocks has no built-in Avalanche staking — but its Raw Signing API can sign any P-Chain transaction. Here's the complete flow, from C-Chain export to delegation, with working TypeScript.
date: 2026-06-09
authors: [owenwg]
...
This post walks the complete flow with working code: derive the P-Chain address, move AVAX from the C-Chain via atomic transfer (export + import), and delegate to a validator. A runnable version of everything below lives at [owenwahlgren/fireblocks-pchain-staking](https://github.com/owenwahlgren/fireblocks-pchain-staking).
...
With AVAX on the P-Chain, staking is one more transaction. `AddPermissionlessDelegatorTx` delegates to an existing validator; the stake and rewards return to `rewardAddresses` automatically when the period ends.
...
const delegateTx = pvm.e.newAddPermissionlessDelegatorTx(
  {
    ...
    subnetId: networkIDs.PrimaryNetworkID.toString(),  // staking on the Primary Network
    start,
...
Poll `pvmApi.getTxStatus({ txID })` until `Committed`, then watch your stake on the [explorer](https://subnets.avax.network/validators).

To run your own validator instead, use `newAddPermissionlessValidatorTx` — it needs 2,000 AVAX and the node's BLS proof of possession from its `info.getNodeID` response.
...
- **Stake is locked** for the full period, with no early unstake. Mainnet delegation minimum is 25 AVAX for 2 weeks; on Fuji it's 1 AVAX for 24 hours, which makes testnet iteration cheap.
...
Two ACPs currently moving through the proposal process would significantly improve the custodial staking story: [ACP-236](/docs/acps/236-continuous-staking) adds auto-renewal and auto-compounding so positions don't need to be manually re-established every period, and [ACP-273](https://github.com/avalanche-foundation/ACPs/blob/main/ACPs/273-reduce-minimum-staking-duration/README.md) cuts the minimum stake duration to roughly two days. Same transaction flow as above — just better parameters.

## Try it

The complete example — including a verification harness that exercises the full signing path against live Fuji data without needing a Fireblocks account — is at [owenwahlgren/fireblocks-pchain-staking](https://github.com/owenwahlgren/fireblocks-pchain-staking). Questions or stuck on enablement? Reach out on the [Avalanche Discord](https://discord.gg/avax).
