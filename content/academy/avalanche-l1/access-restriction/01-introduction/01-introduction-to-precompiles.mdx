...
- **Stateful precompiles**: Can read and write data to the EVM state. The AllowList precompiles we use in this course are stateful—they store role assignments (Admin, Manager, Enabled) for each address. This data persists across blocks.

The Transaction AllowList and Contract Deployer AllowList are **stateful precompiles** because they need to remember which addresses have which permissions.
