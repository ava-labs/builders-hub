...
export function AcademyBlogSection({ blogs }: AcademyBlogSectionProps) {
  ...
  const getTopicColor = (topic: string): string => {
    ...
    if (
      ...
      topicLower.includes("upgrade") ||
      topicLower.includes("etna")
    ) {
    ...
    if (
      topicLower.includes("layer") ||
      topicLower.includes("l1") ||
    ...
    if (
      topicLower.includes("solidity") ||
      topicLower.includes("foundry") ||
      topicLower.includes("smart contract")
    ) {
    ...
    if (
      topicLower.includes("validator") ||
      topicLower.includes("staking") ||
      topicLower.includes("stake")
    ) {
    ...
    // DeFi/Token topics
    if (
      topicLower.includes("token") ||
      topicLower.includes("defi") ||
    ...
    if (
      topicLower.includes("cross") ||
      topicLower.includes("bridge") ||
      topicLower.includes("interop")
    ) {
            ...
            {blog.data.topics && blog.data.topics.length > 0 && (
              <div className="mb-4 flex flex-wrap gap-2">
                {blog.data.topics.slice(0, 2).map((topic, idx) => (
                  <span
                    key={idx}
                    className={cn("text-xs px-2 py-1 rounded-md font-medium", getTopicColor(topic))}
                  >
                    {topic}
                  </span>
                ))}
