...
import {
  ...
  formatTimeRemaining,
  getStatusData,
} from "@/components/toolbox/console/testnet-infra/ManagedTestnetNodes/useTimeRemaining";
...
export default function RelayerCard({
  ...
  const getChainInfo = (config: (typeof relayer.configs)[0]) => {
    ...
    if (
      config.rpcUrl.includes("avax-test.network") ||
      config.subnetId === "11111111111111111111111111111111LpoYY"
    ) {
  ...
  const sendFunds = async (config: (typeof relayer.configs)[0]) => {
    ...
    try {
      ...
      const evmChainId =
        ...
        (config.rpcUrl.includes("avax-test.network")
          ? 43113
          : parseInt(config.blockchainId.slice(0, 8), 16));
  ...
  const getHealthStatus = () => {
    ...
    if (relayer.health.status === "up") {
      return {
        label: "Healthy",
        color:
          "text-green-700 bg-green-50 border-green-300 dark:bg-green-900/20 dark:border-green-700",
      };
    }
    const hasHealthyComponent =
      relayer.health.details &&
    ...
    if (hasHealthyComponent) {
      return {
        label: "Degraded",
        color:
          "text-yellow-700 bg-yellow-50 border-yellow-300 dark:bg-yellow-900/20 dark:border-yellow-700",
      };
    }
    return {
      label: "Unhealthy",
      ...
      <div className="p-4 space-y-4 min-w-0">
        ...
        <div className="space-y-2">
          ...
          <div className="text-xs text-gray-600 dark:text-gray-400 mb-2">
            Ensure the relayer address maintains a positive balance on all configured chains to
