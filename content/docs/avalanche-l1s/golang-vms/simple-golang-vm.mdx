...
AvalancheGo searches for and registers plugins under the `plugins` [directory](/docs/primary-network/configure/configs-flags#--plugin-dir-string).
...
Now, this VM's static API can be accessed at endpoints `/ext/vm/timestampvm` and `/ext/vm/timestamp`. For more details about VM configs, see [here](/docs/primary-network/configure/configs-flags#virtual-machine-vm-configs).
