...
To check if a given chain is done bootstrapping, in another terminal window call [`info.isBootstrapped`](/docs/apis/info-api#infoisbootstrapped) by copying and pasting the following command:
...
For more information on the requests available at these endpoints, please see the [AvalancheGo API Reference](/docs/apis/p-chain/api) documentation.
