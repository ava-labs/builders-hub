...
import { queryClickHouse, C_CHAIN_ID, buildAddressFilter, buildSwapPricesCTE, buildTraceAttributionCTE, getTotalChainGas } from '@/lib/clickhouse';
...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      // 2. Get per-contract stats for the CURRENT period (trace-based attribution)
      queryClickHouse<{
        address: string;
        tx_count: string;
        total_gas: string;
        ...
        ${buildTraceAttributionCTE(allAddresses, timeFilter)}
        SELECT
          ta.address,
          count(DISTINCT ta.tx_hash) as tx_count,
          sum(ta.self_gas) as total_gas,
          sum(toFloat64(ta.self_gas) * toFloat64(t.base_fee_per_gas)) / 1e18 as avax_burned,
          sum(toFloat64(ta.self_gas) * toFloat64(t.base_fee_per_gas) / 1e18 * coalesce(p.price_usd, 0)) as avax_burned_usd,
          sum(toFloat64(ta.self_gas) * toFloat64(t.gas_price) / 1e18 * coalesce(p.price_usd, 0)) as gas_cost_usd,
          uniqExact(t.from) as unique_senders
        FROM trace_attribution ta
        INNER JOIN raw_txs t ON ta.tx_hash = t.hash AND t.chain_id = ${C_CHAIN_ID}
      ...
      // 3. Get per-contract stats for the PREVIOUS period (trace-based attribution)
      days > 0
        ? queryClickHouse<{
            address: string;
            tx_count: string;
            ...
            ${buildTraceAttributionCTE(allAddresses, prevTimeFilter)}
            SELECT
              ta.address,
              count(DISTINCT ta.tx_hash) as tx_count,
              sum(ta.self_gas) as total_gas,
