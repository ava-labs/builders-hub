...
- [getAtomicTxStatus](#getatomictxstatus) - Get transaction status (deprecated)
...
`getAtomicTxStatus` calls `avax.getAtomicTxStatus`, which is deprecated. After the Helicon upgrade, the public API endpoints do not serve it. Use [getAtomicTx](#getatomictx) instead. When the transaction is accepted, the response includes `blockHeight`.
</Callout>
