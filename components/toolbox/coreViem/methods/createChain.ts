...
export async function createChain(
  ...
  const chainIdCollision = existingChains.find((chain) => chain.platformChainId === chainID);

  if (chainIdCollision) {
    throw new Error(
      `Chain ID collision detected. The generated chain ID "${chainID}" already exists.`
    );
  }
