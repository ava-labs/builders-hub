...
```bash
forge create --rpc-url dispatch-l1 --private-key $PK contracts/interchain-messaging/incentivize-relayer/receiverWithFees.sol:ReceiverOnDispatch --broadcast
```
```
[⠊] Compiling...
...
```bash
cast call --rpc-url dispatch-l1 $RECEIVER_ADDRESS "lastMessage()(string)"
```
```bash
"Hello"
```
