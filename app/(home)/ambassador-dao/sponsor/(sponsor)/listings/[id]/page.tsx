...
const JobApplications = ({ listingId }: { listingId: string }) => {
  ...
  const [currentExportKey, setCurrentExportKey] = useState(0);
  ...
  const handleExport = (format: React.SetStateAction<string>) => {
    if (format === "none") return;
    setExportFormat(format);
    queryClient.removeQueries({ queryKey: ["exportCsv", listingId, format] });
    setExporting(true);
  };
  ...
  useEffect(() => {
    if (isSuccess || error) {
      ...
      setCurrentExportKey((prev) => prev + 1);
    }
  }, [isSuccess, error]);
    ...
    <div className='border border-[var(--default-border-color)] rounded-md p-3 md:p-6'>
      <div className='flex justify-between items-center mb-6'>
        ...
        <div className='flex space-x-2'>
          <Select
            key={`export-select-${currentExportKey}`}
            defaultValue='none'
            onValueChange={(value) => {
              handleExport(value);
              queryClient.removeQueries({
                queryKey: ["exportCsv", listingId, value],
              });
              setExporting(true);
            }}
