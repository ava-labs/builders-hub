...
const isValidAllowlistPrecompileConfig = (config: AllowlistPrecompileConfig): boolean => {
  ...
  if (
    config.addresses.Admin.filter((entry: AddressEntry) => !entry.requiredReason && !entry.error)
      .length === 0 &&
    config.addresses.Manager.filter((entry: AddressEntry) => !entry.requiredReason && !entry.error)
      .length === 0 &&
    config.addresses.Enabled.filter((entry: AddressEntry) => !entry.requiredReason && !entry.error)
      .length === 0
  ) {
    return false;
  }
