  ...
  useEffect(() => {
    const extractWarpMessage = async () => {
      ...
      try {
        ...
        // The unsigned warp message is simply the data field from the first log
        const unsignedWarpMessage = receipt.logs[0].data;

        if (!unsignedWarpMessage) {
          throw new Error("Could not extract warp message from transaction receipt data field.");
        }
