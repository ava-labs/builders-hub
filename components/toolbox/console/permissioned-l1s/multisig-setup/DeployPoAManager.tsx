    ...
    <>
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <SelectSubnetId value={subnetIdL1} onChange={setSubnetIdL1} hidePrimaryNetwork={true} />
        ...
        {Boolean(subnetIdL1) && (
          <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded text-yellow-800 dark:text-yellow-200 text-sm">
            <strong className="font-semibold">Heads up:</strong> Make sure your Validator Manager is deployed on the
            same chain as your{' '}
            <a href="https://wallet.ash.center/" target="_blank" rel="noopener noreferrer" className="underline">
              Ash Wallet
            </a>{' '}
            deployment. You can get your L1 indexed to ensure full compatibility. You can proceed with other multisig
            options, but some console tools may lose compatibility if you do.
          </div>
        )}
        ...
        <Steps>
          <Step>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Deploy the <code>PoAManager</code> contract with the specified owner and validator manager addresses. The
              contract will be initialized automatically during deployment.
            </p>
            {viemChain && (
              ...
              <div className="space-y-3">
                ...
                {safeSelection.safeAddress && (
                  <div className="text-xs text-zinc-600 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-800 p-2 rounded-lg">
                    ...
                    {safeSelection.owners.length > 0 ? safeSelection.owners.join(', ') : 'Loading...'}
                  </div>
                )}
              </div>
            ...
            {poaManagerAddress && <Success label="PoA Manager Deployed & Initialized" value={poaManagerAddress} />}
          </Step>
          ...
          <Step>
            ...
            <div className="space-y-4">
              <EVMAddressInput label="PoA Manager Address" value={poaManagerAddress} onChange={setPoaManagerAddress} />

              <Button variant="secondary" onClick={checkIfInitialized} loading={isChecking} size="sm">
                Verify Contract
              </Button>

              {isInitialized === true && (
                <div className="space-y-2">
                  <Success label="Contract Verified" value={`Owner: ${verifiedOwner ?? ownerAddress}`} />
                </div>
              )}
