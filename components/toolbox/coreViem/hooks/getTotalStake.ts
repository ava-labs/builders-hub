...
export function validateStakePercentage(
  ...
  if (
    totalL1StakeBeforeChange === 0n &&
    currentWeightOfValidatorToChange === 0n &&
    newProposedWeightForValidator > 0n
  ) {
  ...
  const percentageOfTotalRepresentedByChange = Number((weightAdjustment * 10000n) / totalL1StakeBeforeChange) / 100;
