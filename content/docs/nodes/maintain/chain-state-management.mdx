...
| Property | Active State | Archival State |
|----------|--------------|----------------|
| **Size (C-Chain)** | ~500 GB | ~3 TB+ (and growing) |
| **Contents** | Current account balances, contract storage, code | Complete state history at every block |
| **Required for** | Validating, sending transactions, reading current state | Historical queries at any block height, block explorers, analytics |
| **Sync method** | State sync (fast, hours) | Full sync from genesis (slow, days) |
| **Maintenance** | Periodic pruning or resync recommended | None needed (intentional full history) |
